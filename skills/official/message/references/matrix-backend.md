...
- Codex MCP registration: `mcp_servers.matrix-hub` in
  `/Users/carlvincetan/.codex/config.toml`
...
`Matrix Hub Codex Token`. Never print it. Each bridge keeps its own credentials
...
After changing or newly registering the MCP server, a running Codex task may
...
stdio, but it does not prove that the current Codex task reloaded its tool
catalog.
