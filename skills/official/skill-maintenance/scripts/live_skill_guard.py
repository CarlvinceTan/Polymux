      ...
      "hooks": [
        {
          ...
          "command": "python3 ~/.midas/skills/skill-maintenance/scripts/live_skill_guard.py"
        }
      ]
    }
...
review paths under ~/.midas/skill-maintenance remain writable.
"""
...
LIVE_ROOTS = [Path.home() / ".midas" / "skills", Path.home() / ".agents" / "skills"]
ALLOWED_ROOT = Path.home() / ".midas" / "skill-maintenance"
...
def candidate_paths(tool: str, arguments: dict) -> list[Path]:
    ...
    if tool in ("write", "edit"):
        ...
        for match in re.findall(r"(?:~|/Users/[^\s'\"]+)/\.(?:midas|agents)/skills[^\s'\"]*", command):
