// Midas browser extension — background service worker.
//
// Two responsibilities:
//  1. Tab context: stream the open-tab list to the Midas desktop agent through
//     the com.midas.tab_context native messaging host (tabs.json on disk).
//  2. Agent surface: proxy the loopback Midas feed for content scripts —
...
const HOST = "com.midas.tab_context";
...
chrome.runtime.onInstalled.addListener(() => {
  chrome.alarms.create("midas-tab-context-heartbeat", {
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === "midas-tab-context-heartbeat") scheduleSnapshot();
});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message?.type === "midas:cursor-arrived") {
    return respondWith(
      fetch(ARRIVAL_URL, {
        method: "POST",
        cache: "no-store",
  ...
  if (message?.type === "midas:command-result") {
    return respondWith(
      fetch(RESULTS_URL, {
        method: "POST",
        cache: "no-store",
  ...
  if (message?.type === "midas:snapshot" || message?.type === "midas:changes") {
    const url = new URL(SNAPSHOT_URL);
    if (
      message.type === "midas:changes" &&
    ...
    return respondWith(
      fetch(url.href, { cache: "no-store" }).then(async (response) => {
        if (!response.ok)
          throw new Error(`Midas agent surface returned ${response.status}`);
