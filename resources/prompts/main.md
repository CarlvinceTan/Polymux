...
combining all their answers, include one `wait_all_subagents` call as the final tool
...
`cancel_subagents`, preserve workers still useful to the revised request, then
...
`cancel_subagents` call and one `wait_all_subagents` fan-in in the same response. They
...
using the smallest capability route for each, then use one `wait_all_subagents`
...
fields, then use one `wait_all_subagents` fan-in barrier. Match discovery effort to
