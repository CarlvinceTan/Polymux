...
app_pids() {
  ...
  if [[ -n "$bundle_id" ]]; then
    ...
    bundle_pids=()
    # `lsappinfo find` returns multiple matching ASNs space-separated on one
    # line. Splitting only on newlines silently kept the first PID and made a
    # correctly launched second instance look unverified.
...
matching_pids() {
  ...
  # Launch Services may deliberately omit an Electron app's argv from
  # `ps -o command`, returning only its executable name. For a route just
  # resolved from the compiled registry, the immutable route arguments plus a
  # new exact-bundle PID are stronger evidence than that lossy display. This
  # branch is enabled only after compiled lookup proves the requested marker is
  # one of the registered launch arguments; existing and ad-hoc processes keep
  # using the command-line check below.
  if [[ "$compiled_verifies_command" == "true" ]]; then
    # Ambiguous concurrent launches fail closed: the compiled command must
    # account for exactly one new main application PID.
    unique_new_csv_pid "$app_pids_before_snapshot" "$pid_csv" 2>/dev/null || print -r -- ""
    return 0
  fi
...
if ! app_pids_before_snapshot="$(app_pids)"; then
  report "blocked_state_unavailable"
  exit 6
fi
...
if [[ "$mode" == "check" ]]; then
  ...
  if [[ "$app_running_before" == "true" ]]; then
    ...
    elif [[ -n "$verify_command_contains" && -z "$matching_pids_before" ]]; then
...
if [[ "$strict_recovery_result" == *'"status":"recovered"'* ]]; then
  ...
  if [[ "$first_use_launch" == "true" ]]; then
    ...
    remember_command=(/usr/bin/python3 "${0:A:h}/app-control-registry.py" remember-route
      --kind foregrounding
      --app "$app_name"
      --bundle-id "$bundle_id"
      --launcher "$verified_launch_command")
    for launch_arg in "${verified_launch_args[@]}"; do
      remember_command+=("--arg=$launch_arg")
    done
    "${remember_command[@]}" >/dev/null 2>&1 || true
  fi
  report "ready_background_recovered_launch"
  exit 0
fi
