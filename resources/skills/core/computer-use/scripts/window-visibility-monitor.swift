...
func significantOnscreenWindows(options: Options) -> [[String: Any]] {
    ...
    return currentRows.filter { row in
        guard let pid = (row[kCGWindowOwnerPID as String] as? NSNumber)?.int32Value,
              ...
              // Electron can keep an exact accessibility surface available by
              // showing an alpha-zero, mouse-ignoring window. Core Graphics
              // calls that window onscreen even though it cannot expose pixels
              // to the user. Only visible alpha is an exposure incident.
