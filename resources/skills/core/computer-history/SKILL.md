...
description: Answer what the user was doing from Polymux's built-in ComputerHistory recorder. Search screen history with the narrowest identifying phrase, read the smallest useful time range, verify names and payloads through the owning file, connector, or domain skill, and label conclusions verified, screen-only, or inferred. Retrieval only; it never changes recorder settings, capture policy, or external state.
author: Polymux
---
...
ComputerHistory lives under `~/Library/Application Support/Polymux/computer-history/` and
...
[evidence-quality.md](../window-control/references/evidence-quality.md). It contains
