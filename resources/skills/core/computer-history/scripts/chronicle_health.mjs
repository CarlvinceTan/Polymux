...
function recorderState(root) {
  ...
  if (!isDir) {
    errors.push("ComputerHistory directory is missing; ComputerHistory has never run");
    return [state, errors];
  }
  try {
    const value = JSON.parse(readFileSync(path.join(root, "settings.json"), "utf8"));
    state.enabled = Boolean(value.enabled ?? true);
    // What the user allowed ComputerHistory to see. Reported rather than judged:
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    if (argv[i] === "--computerHistory-root") out.computerHistoryRoot = argv[++i];
...
const root = args.computerHistoryRoot;
...
const warnings = [];
if (!frame.available) errors.push("No ComputerHistory frame index is available");
else if (!frame.fresh) warnings.push("Newest ComputerHistory frame is stale");
if (!timeline.available) warnings.push("No ComputerHistory timeline is available");
else if (!timeline.fresh) warnings.push("ComputerHistory timeline is stale");
if (recorder.interaction_events === false) {
  warnings.push("Interaction events are switched off; only window text is recorded");
} else if (!events.available) warnings.push("No ComputerHistory interaction events are available");
