...
def recorder_state(root: Path) -> tuple[dict, list[str]]:
    ...
    if not root.is_dir():
        errors.append("ComputerHistory directory is missing; ComputerHistory has never run")
        return state, errors
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.home() / "Library/Application Support/Polymux/computer-history",
    )
...
def main() -> int:
    args = parse_args()
    now = time.time()
    root = args.computerHistory_root
    ...
    if not frame["available"]:
        errors.append("No ComputerHistory frame index is available")
    elif not frame["fresh"]:
        warnings.append("Newest ComputerHistory frame is stale")
    if not timeline["available"]:
        warnings.append("No ComputerHistory timeline is available")
    elif not timeline["fresh"]:
        warnings.append("ComputerHistory timeline is stale")
    if recorder.get("interaction_events") is False:
        ...
        warnings.append("No ComputerHistory interaction events are available")
