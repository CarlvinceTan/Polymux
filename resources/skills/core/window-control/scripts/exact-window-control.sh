...
# Renew rather than validate: `renew` runs the same expiry and token checks and
# refuses on the same terms, and additionally pushes the expiry out. Doing it
# here means the lease measures time since the last action rather than time
# since it was taken — which is what a lease on live work should mean. Under
# `validate` the 300s TTL ran from acquisition, so work that legitimately took
# longer hit an expiry the skill treats as a hard stop, and the menu-bar pill
# went out while the agent was still driving.
if [[ "$command_name" != "list" ]]; then
  if ! lease_output="$(/usr/bin/python3 "$lease_script" renew \
    --app-id "$app_id" \
