...
# Some apps expose an app-owned launch contract that is stricter than the
# generic macOS background hint. Keep these requirements pinned to the exact
...
def required_app_launch_args(bundle_id: str) -> list[str]:
    return list(REQUIRED_APP_LAUNCH_ARGS.get(bundle_id, []))
...
def background_open_args(path: Path, bundle_id: str) -> list[str]:
    ...
    if bundle_id in NEW_INSTANCE_BUNDLE_IDS:
        ...
        args.append("-j")
    args.extend(["-a", str(path)])
    return args


def route_has_required_app_args(route: dict) -> bool:
    required = required_app_launch_args(str(route.get("bundle_id", "")))
    route_args = route.get("args", [])
    if not isinstance(route_args, list):
        return False
    try:
        separator = route_args.index("--args")
    except ValueError:
        return not required
    return all(item in route_args[separator + 1 :] for item in required)
...
def quarantined_for_app(
    ...
    """
    Whether this exact app build has already misbehaved here, in this way.
    Build identity is part of the first-use route id: a repaired build must earn
    trust again, while the failed binary remains quarantined indefinitely.
    """
...
def running_bundle_pids(bundle_id: str) -> set[int]:
    ...
    if result.returncode != 0:
        return set()
    pids: set[int] = set()
    # LaunchServices puts multiple ASN records on one line. Match each full
    # record independently, including quoted names containing spaces, and use
    # only its numeric ASN identity for the PID query.
    records = re.finditer(
        r'(?<!\S)(ASN:0x[0-9a-fA-F]+-0x[0-9a-fA-F]+)'
        r'(?:-"(?:[^"\\]|\\.)*":)?(?=\s|$)',
        result.stdout,
    )
    for token in dict.fromkeys(match.group(1) for match in records):
        info = subprocess.run(
        ...
        if info.returncode != 0:
            continue
        match = re.fullmatch(r'"pid"=([1-9][0-9]*)', info.stdout.strip())
        if match:
...
def lookup_launch(args: argparse.Namespace) -> int:
    ...
    matches = [
        ...
        and route_has_required_app_args(route)
    ]
...
def offer_first_use_launch(args: argparse.Namespace) -> int:
    ...
    incident = quarantined_for_app(
        Path(args.state_dir), args.bundle_id or args.app, "launch", route_id
    )
    if incident:
        emit(
            "blocked_route_quarantined",
        ...
        return 5
    launch_args = background_open_args(path, identity["bundle_id"])
    required_args = required_app_launch_args(identity["bundle_id"])
    if required_args:
        launch_args.extend(["--args", *required_args])
    emit(
        "first_use_monitored",
        route_id=route_id,
...
def offer_first_use_control(args: argparse.Namespace) -> int:
    ...
    incident = quarantined_for_app(
        Path(args.state_dir), args.bundle_id or args.app, "control", route_id
    )
    if incident:
        emit(
            "blocked_route_quarantined",
...
def remember_route(args: argparse.Namespace) -> int:
    ...
    if kind == "control_routes":
        ...
        default_args = background_open_args(path, identity["bundle_id"])
        required_args = required_app_launch_args(identity["bundle_id"])
        if required_args:
            default_args.extend(["--args", *required_args])
        route["args"] = args.arg or default_args
        if not route_has_required_app_args(route):
            emit("blocked_required_launch_args_missing")
            return 8
