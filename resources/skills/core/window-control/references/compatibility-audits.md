...
Before testing, query `scripts/runtime-route-observations.py lookup` with the
exact app path and requested capability.

```bash
python3 scripts/runtime-route-observations.py lookup \
...
```bash
python3 scripts/runtime-route-observations.py record \
  --app-path /absolute/App.app --capability press \
