...
Opens your `$EDITOR` and may offer both save and send actions. Avoid this route
for unattended draft creation because the final choice is interactive:
...
```bash
cat message.txt | himalaya template save -a ACCOUNT --folder Drafts
```
...
- Prefer a deterministic template saved to Drafts over an interactive editor.
