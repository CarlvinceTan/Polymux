...
Provider-neutral inference contracts for Polymux, plus a thin `pi-ai` implementation.
...
for await (const event of inference.stream({
  ...
  // Feed typed events into the Polymux agent core.
}
```

The public contract owns model references, messages, tools, streaming events, usage and errors. Pi-specific model collections and provider setup are exposed only from `@polymux/inference/pi`.
