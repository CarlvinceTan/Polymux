...
export class EmailAccounts {
  ...
  async message(options: {
    ...
    for (const header of ["From", "To", "Cc", "Bcc", "Subject", "Date", "Message-ID", "References", "In-Reply-To"])
      args.push("--header", header);
    if (options.folder) args.push("--folder", options.folder);
    if (options.account) args.push("--account", options.account);
    // `message read` renders a message *for a terminal*: it converts HTML to
    // text and never prints the markup, so a mail that is only HTML — which is
    // most of them — arrives as the sender's thin plain-text alternative, or
    // as flattened text. `message export` is the one command that writes the
    // MIME parts out as they are, so the two run together and the export wins
    // wherever it has HTML. They are fetched concurrently: the reader waits
    // for the slower of the two rather than for both in turn.
    const [raw, exported] = await Promise.all([
      this.#jsonCommand(args),
    ...
    const body =
      typeof raw === "string"
        ...
        : isTable(raw) && typeof raw.message === "string"
          ? raw.message
          : JSON.stringify(raw);
    const message = parseMessage(options.id, body);
    return exported ? {...message, html: exported} : message;
  }

  /**
   * The message's own HTML part, via an export into a directory of our own.
   * Best-effort by design: a text-only message has no such part, an older
   * Himalaya may not carry the command, and either answer is simply "no HTML"
   * rather than a message that fails to open.
   */
  async #exportHtml(options: {id: string; account?: string; folder?: string}): Promise<string | null> {
    let dir = "";
    try {
      dir = await mkdtemp(path.join(tmpdir(), "flareai-mail-"));
      const args = ["message", "export", "--destination", dir];
      if (options.folder) args.push("--folder", options.folder);
      if (options.account) args.push("--account", options.account);
      args.push(options.id);
      const result = await this.#run(this.#binary, ["-c", this.#configPath, ...args]);
      if (result.code !== 0) return null;
      // One file per MIME part, named after its type: `index.html` is the
      // HTML body wherever the message has one.
      const files = await readdir(dir);
      const name = files.find((file) => file.toLowerCase().endsWith(".html"));
      if (!name) return null;
      const html = (await readFile(path.join(dir, name), "utf8")).trim();
      return html || null;
    } catch {
      return null;
    } finally {
  ...
  async send(options: {
    ...
    attachments?: string[];
    /** "high" or "low" marks the message's priority; "normal" says nothing. */
...
function leafName(name: string): string {
  ...
  // IMAP names the inbox "INBOX"; shouting it beside Drafts and Sent Mail
  // would be the only all-caps row in the list.
