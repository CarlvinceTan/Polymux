...
function frame(signature: number[], image = "image"): ComputerHistoryFrame {
  return {
    sourceId: "screen:1:0",
...
const active: ComputerHistorySystemState = {
  idleSeconds: 0,
  locked: false,
  onBattery: false,
...
function windowFrame(
  overrides: Partial<ComputerHistoryFrame> & { sourceId: string },
...
test("records every app and site until the user narrows it", async () => {
  ...
  const captured = async (patch?: Partial<ComputerHistorySettings>) => {
...
test("interaction events are searchable and obey the same policy", () => {
  // ComputerHistory is on by default, so no start() here: starting the manager would
...
test("removing one frame leaves neighbouring captures intact", async () => {
  let now = new Date("2026-08-13T12:00:00.000Z");
  let next = windowFrame({sourceId: "notes", app: "Notes"});
  const manager = new ComputerHistoryManager({
    directory: mkdtempSync(path.join(tmpdir(), "polymux-computer-history-")),
  ...
  const first = (await manager.captureOnce())[0]!;
  now = new Date("2026-08-13T12:01:00.000Z");
  next = windowFrame({sourceId: "notes", app: "Notes", image: Buffer.from("later")});
  const second = (await manager.captureOnce())[0]!;

  const status = manager.removeEntry(first.id);
  assert.equal(status.storedFrames, 1);
  assert.equal(manager.store.entries()[0]?.id, second.id);
