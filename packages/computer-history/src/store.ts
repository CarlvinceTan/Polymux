...
export const defaultComputerHistorySettings: ComputerHistorySettings = {
  enabled: true,
...
export class ComputerHistoryStore {
  ...
  constructor(directory: string) {
    ...
    if (!existsSync(this.#settingsPath)) this.writeSettings(defaultComputerHistorySettings);
    // Rewritten every start rather than only when absent: the retrieval rules
    // change with the code, and a stale copy would describe a ComputerHistory that
    // no longer exists.
  ...
  readSettings(): ComputerHistorySettings {
    try {
      return validateSettings(JSON.parse(readFileSync(this.#settingsPath, "utf8")));
    } catch {
      return { ...defaultComputerHistorySettings };
    }
  }

  writeSettings(settings: ComputerHistorySettings): void {
    writeFileSync(
      this.#settingsPath,
  ...
  save(
    frame: ComputerHistoryFrame,
    capturedAt: Date,
    change: number,
    reason: ComputerHistoryEntry["reason"],
  ): ComputerHistoryEntry {
    ...
    const entry: ComputerHistoryEntry = {
      id,
  ...
  entries(options: { since?: Date; until?: Date; limit?: number } = {}): ComputerHistoryEntry[] {
    ...
    const entries = readdirSync(this.indexDirectory, { withFileTypes: true })
      ...
      .flatMap((entry) => readLines<ComputerHistoryEntry>(path.join(this.indexDirectory, entry.name)))
      .filter((entry) => {
  ...
  search(
    ...
    const hits: ComputerHistorySearchHit[] = [];
  ...
  /** Removes exactly one indexed frame and refreshes every derived view. */
  removeEntry(id: string): boolean {
    const entry = this.entries({limit: Number.MAX_SAFE_INTEGER}).find((candidate) => candidate.id === id);
  ...
  prune(now: Date, settings: ComputerHistorySettings): void {
  ...
  status(settings: ComputerHistorySettings, running: boolean, lastError: string | null): ComputerHistoryStatus {
  ...
  #compactIndexes(): void {
    for (const item of readdirSync(this.indexDirectory, { withFileTypes: true })) {
      ...
      const active = readLines<ComputerHistoryEntry>(index).filter((entry) => existsSync(entry.path));
...
function validateSettings(value: Partial<ComputerHistorySettings>): ComputerHistorySettings {
  return {
...
function instructions(directory: string): string {
  return `# ComputerHistory retrieval

ComputerHistory is private local screen evidence stored under \`${directory}\`.
