...
import { ComputerHistoryStore } from "./store.js";

export interface ComputerHistoryManagerOptions {
  directory: string;
  frames: ComputerHistoryFrameSource;
  system: ComputerHistorySystemStateSource;
...
export class ComputerHistoryManager {
  readonly store: ComputerHistoryStore;
  readonly #frames: ComputerHistoryFrameSource;
  readonly #system: ComputerHistorySystemStateSource;
  ...
  readonly #clock: () => Date;
  readonly #schedule: NonNullable<ComputerHistoryManagerOptions["schedule"]>;
  readonly #cancelSchedule: NonNullable<ComputerHistoryManagerOptions["cancelSchedule"]>;
  ...
  constructor(options: ComputerHistoryManagerOptions) {
    this.store = new ComputerHistoryStore(options.directory);
  ...
  promptContext(): ComputerHistoryPromptContext {
    return {
  ...
  setEnabled(enabled: boolean): ComputerHistoryStatus {
   ...
   * writes.
   */
  update(patch: Partial<ComputerHistorySettings>): ComputerHistoryStatus {
  ...
  forget(since: Date, until: Date): ComputerHistoryStatus {
  ...
  removeEntry(id: string): ComputerHistoryStatus {
    if (this.store.removeEntry(id)) this.#previous.clear();
    return this.status();
  }

  async captureOnce(): Promise<ComputerHistoryEntry[]> {
    ...
    try {
      ...
      const saved = frames.flatMap((frame) => {
        ...
        const heartbeat = previous
          ...
          : false;
        const reason: ComputerHistoryEntry["reason"] | null = !previous
          ? "initial"
