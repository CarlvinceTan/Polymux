...
import { VaultSession, type VaultBlobStore } from "../src/session.js";
...
test("a portable session creates, unlocks, and fills without Node APIs in totp", async () => {
  ...
  const session = new VaultSession(store);
  ...
  session.lock();
  const again = new VaultSession(store);
  await again.hydrate();
