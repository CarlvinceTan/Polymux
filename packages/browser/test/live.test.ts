...
describe("browser against a real browser", { skip: !available && (requested ? "no Chrome installed" : "set POLYMUX_LIVE_BROWSER=1 to run") }, () => {
  ...
  test("a stale ref is refused rather than resolved to something else", async () => {
    const first = await handlers.snapshot(session, {});
    const stale = /\[ref=(e\d+)\]/.exec(first.content)?.[1];
    assert.ok(stale, "first snapshot had no ref");
    await handlers.snapshot(session, {});
    await assert.rejects(
      () => handlers.click(session, { ref: stale }),
      /Unknown ref/,
    );
  });
