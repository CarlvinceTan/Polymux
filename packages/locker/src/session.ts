...
import type { LockerCloudBlob } from "./cloud.js";
...
export interface LockerBlobStore {
  load(): Promise<{ bytes: Uint8Array; meta: VaultMeta } | null>;
  save(bytes: Uint8Array, meta: VaultMeta): Promise<void>;
}

export interface LockerVaultBlob {
  bytes: string;
  meta: VaultMeta;
}

export interface LockerSessionStatus {
  exists: boolean;
  unlocked: boolean;
  itemCount: number;
...
export interface LockerSessionSecrets {
  password: string;
  totp: ReturnType<typeof totpFor>;
  recoveryCodes: string[];
  passkey: Omit<NonNullable<LockerSecrets["passkey"]>, "privateKeyPem"> | null;
}

/** In-memory KeePass session over a portable encrypted-blob store. */
export class LockerSession {
  readonly #store: LockerBlobStore;
  #bytes: Uint8Array | null = null;
  #meta: VaultMeta = emptyMeta();
  #db: Kdbx | null = null;
  #idle: ReturnType<typeof setTimeout> | null = null;

  constructor(store: LockerBlobStore) {
    this.#store = store;
  }

  async hydrate(): Promise<LockerSessionStatus> {
  ...
  async create(password: string): Promise<LockerSessionStatus> {
  ...
  async unlock(password: string): Promise<LockerSessionStatus> {
  ...
  reveal(id: string): LockerSessionSecrets {
    const db = this.#requireOpen();
    this.#armIdle();
  ...
  matchesForUrl(url: string): LockerItem[] {
    if (!this.#db) return [];
    this.#armIdle();
  ...
  async save(input: LockerItemInput): Promise<LockerItem> {
    const db = this.#requireOpen();
  ...
  async changePassword(current: string, next: string): Promise<LockerSessionStatus> {
  ...
  async setStorage(mode: VaultStorage): Promise<LockerSessionStatus> {
  ...
  exportBlob(): LockerVaultBlob | null {
    if (!this.#bytes) return null;
    return { bytes: bytesToBase64(this.#bytes), meta: this.#meta };
  }

  async importBlob(blob: LockerVaultBlob): Promise<LockerSessionStatus> {
  ...
  async importCloud(blob: LockerCloudBlob): Promise<LockerSessionStatus> {
  ...
  async acknowledgePush(): Promise<LockerSessionStatus> {
...
export function parseLockerVaultBlob(value: unknown): LockerVaultBlob | null {
