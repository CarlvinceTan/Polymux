...
/** Locker-only pin and grid order. KeePass stores them as ordinary extra fields. */
...
export const LOCKED = "Locker is locked";
export const MISSING_ITEM = "That item is not in the locker";
...
export interface LockerSecrets {
  password: string;
...
export interface LockerItemInput {
  id?: string;
  title: string;
  username?: string;
  url?: string;
...
export async function createDatabase(password: string, name = "Locker"): Promise<kdbxweb.Kdbx> {
  installArgon2();
...
export function listGroups(db: kdbxweb.Kdbx): LockerGroup[] {
...
export function listItems(db: kdbxweb.Kdbx): LockerItem[] {
...
export function listTrash(db: kdbxweb.Kdbx): LockerItem[] {
  const items: LockerItem[] = [];
...
export function readItem(db: kdbxweb.Kdbx, id: string): LockerItem | null {
  const found = findEntry(db, id);
  return found ? summarize(found.entry, found.group) : null;
}

export function readSecrets(db: kdbxweb.Kdbx, id: string): LockerSecrets {
  const found = findEntry(db, id);
...
export function upsertItem(db: kdbxweb.Kdbx, input: LockerItemInput): string {
...
export function importCsvLogins(db: kdbxweb.Kdbx, text: string): LockerImportResult {
...
function compareItems(a: LockerItem, b: LockerItem): number {
...
function summarize(entry: kdbxweb.KdbxEntry, group: kdbxweb.KdbxGroup): LockerItem {
...
function readPasskey(entry: kdbxweb.KdbxEntry): LockerPasskey | null {
...
function writePasskey(
  ...
  passkey: LockerItemInput["passkey"],
): void {
  if (!passkey) {
    for (const field of [
      PASSKEY_RP,
