...
test("cloud pull downloads ciphertext and never includes a master password", async () => {
  ...
  const account = new PolymuxAccountClient({
    ...
    fetch: async (url, init) => {
      ...
      if (href.includes(`/rest/v1/${LOCKER_CLOUD_TABLE}`)) {
        return new Response(
          JSON.stringify([
            {
              revision: 4,
      ...
      if (
        href.includes(
          `/storage/v1/object/${LOCKER_CLOUD_BUCKET}/user-1/${objectName}`,
        )
      ) {
  ...
  const cloud = new FetchLockerCloud(account);
...
test("cloud push uploads the encrypted blob and upserts vault metadata", async () => {
  ...
  assert.equal(
    methods.some(
      (line) =>
        line.startsWith("POST") &&
        line.includes(`/rest/v1/${LOCKER_CLOUD_TABLE}`),
    ),
    true,
  );
});
...
test("cloud push never commits metadata when the immutable upload fails", async () => {
  ...
  const account = new PolymuxAccountClient({
    ...
    fetch: async (url) => {
      ...
      if (String(url).includes(`/rest/v1/${LOCKER_CLOUD_TABLE}`))
        metadataWrites += 1;
      return new Response("{}", {
        status: 200,
        headers: { "content-type": "application/json" },
  ...
  const account = new PolymuxAccountClient({
    ...
    fetch: async (url, init) => {
      ...
      if (href.includes("/storage/v1/object/") && init?.method === "GET")
        return new Response(bytes, { status: 200 });
      if (href.includes(`/rest/v1/${LOCKER_CLOUD_TABLE}`)) metadataWrites += 1;
      return new Response("{}", {
        status: 200,
        headers: { "content-type": "application/json" },
      });
...
test("cloud pull rejects ciphertext that does not match the committed metadata", async () => {
  ...
  await account.restore();
  await assert.rejects(
    new FetchLockerCloud(account).pull(),
