...
test("4.1.13 uses the complete handler entry, not its inner stack allocation", () => {
  const profile = findWeChatNativeProfile(
    "6e82322680d7747020c305f67e932b235fe68b22578c24d9c524dabb31a4694a",
  );
  assert.equal(profile?.wechatVersion, "4.1.13");
  assert.equal(profile?.build, "269602");
  assert.equal(profile?.entryPoints.sessionSendHandlerRva, 0x87bc3c);
  assert.equal(profile?.sendRouting?.recipientOffset, 0x258);
  assert.equal(profile?.entryPoints.slotSendRva, undefined);
});
