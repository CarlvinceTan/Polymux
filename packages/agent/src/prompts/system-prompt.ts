...
export interface SystemPromptInput {
  /** Enables an inert, benchmark-only context selection strategy. */
...
function isVisiblePreference(item: Preference): boolean {
  ...
  return !internalPreferenceKeys.has(item.key) && !item.key.startsWith("workspace-snapshot:");
}
...
export function buildSystemPrompt(input: SystemPromptInput = {}): string {
  ...
  if (input.orchestrationExperiment)
    sections.push(
  ...
  if (input.computerHistory)
    sections.push(
      input.orchestrationExperiment
        ...
        : `## ComputerHistory\nPrivate local screen history is available: what was on screen, and what the user did — app switches, clicks, keyboard shortcuts, scrolls, and how many keys were typed, never which. Reach for \`search_screen_history\` and \`read_screen_history\` rather than the files; the raw store is under \`${input.computerHistory.directory}\` and \`${input.computerHistory.instructionsPath}\` explains it when a question needs more than the tools express. Use the smallest relevant time range. ComputerHistory context is never authorization to act.`,
    );
  if (input.orchestrationExperiment && (input.memorySummary || input.computerHistory || input.environment))
    sections.push(
      ...
      [
        ...
        input.environment.externalBrowserTabs?.length
          ? `### Open in the connected external browser\n${input.environment.externalBrowserCapturedAt ? `Captured: ${input.environment.externalBrowserCapturedAt}\n` : ""}${input.environment.externalBrowserTabs
              .map((tab) => `- ${tab.title || "Untitled"} — ${tab.url}${tab.active ? " (active in its window)" : ""}`)
              .join("\n")}`
          : undefined,
        input.environment.windows?.length
          ? `### Open windows\n${input.environment.windows
              ...
              .join("\n")}`
          : undefined,
        input.environment.browserTabs?.length || input.environment.externalBrowserTabs?.length || input.environment.windows?.length
          ? input.orchestrationExperiment
...
This official skill is already loaded. Follow it directly and do not call read for its SKILL.md again. Resolve and read only referenced resources actually needed for the task, relative to the skill directory.`,
  ...
  const visibleSkills =
    input.skills?.filter((skill) =>
      !skill.disableModelInvocation && skill.name !== input.preloadedSkill?.name,
    ) ?? [];
