import type { ChronicleEntry, InteractionEvent } from "@flareai/chronicle";
...
export const defaultChronicleDistillationSettings: ChronicleDistillationSettings = {
  enabled: true,
  minimumFrames: 8,
 ...
 * twice.
 */
export class ChronicleDistiller {
  readonly #inference: InferenceService;
  readonly #memory: MemoryManager;
  readonly #chronicle: ChronicleAccess;
  readonly #settings: ChronicleDistillationSettings;
  readonly #clock: () => Date;
  #running = false;
  ...
  async maybeDistill(model: ModelRef, signal: AbortSignal): Promise<number> {
    ...
    const store = this.#chronicle.store;
    const settings = this.#chronicle.settings?.();
    ...
    for await (const event of this.#inference.stream({
      ...
      if (event.type === "done")
        answer = event.message.content
          ...
          .join("\n");
      if (event.type === "error")
        throw new Error(`Chronicle distillation failed: ${event.error.message}`);
    }
