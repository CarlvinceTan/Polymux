const STOP_WORDS = new Set([
  "a", "an", "and", "anything", "are", "before", "can", "explain", "find", "for", "from", "has", "have", "i", "in", "is",
  "it", "me", "my", "next", "of", "on", "or", "plans", "request", "send", "sending", "that", "the", "thing", "this", "to", "two", "was", "what",
  "with", "work", "you", "whether", "matters",
]);

const TOPIC_GROUPS = [
  ["reply", "message", "email", "draft", "send", "sending", "recipient", "contact", "dad", "father", "parent", "mum", "mother", "whatsapp", "wechat", "mail"],
  ["career", "job", "application", "role", "interview", "graduate", "internship", "work-authorisation", "sponsorship"],
  ["singapore", "nus", "nusync", "exchange", "student", "pass", "housing"],
  ["travel", "flight", "hotel", "visa", "itinerary", "booking"],
  ["event", "events", "club", "clubs", "society", "societies", "interest", "interested"],
  ["form", "fill", "submit", "field"],
  ["file", "folder", "move", "document", "drive", "upload"],
  ["screen", "window", "tab", "page", "foreground", "computerHistory", "switched", "recent", "earlier", "left-off"],
  ["memory", "remember", "history", "previous", "recall", "preference", "preferences", "interested", "usual", "normally"],
  ["request", "client", "model", "api", "endpoint", "provider", "opencode", "server", "inference"],
];
const ROUTE_INTENT = /\b(?:send|make|try|run) (?:it )?(?:a |one )?request\b|\b(?:client|model|api|endpoint|provider|inference)\b/i;
const COMMUNICATION_REFERENCE = /\b(?:reply|message|email|draft|recipient|contact|dad|father|parent|mum|mother|whatsapp|wechat|mail|inbox)\b/i;
const PERSONALIZATION_REFERENCE = /\b(?:i might be interested|i might (?:like|enjoy)|might i (?:like|enjoy)|would i (?:like|enjoy)|pick(?: one)? for me|choose(?: one)? for me|recommend(?: something| one|ations?)? for me|suggest(?: something| one)? for me|based on my (?:interests|preferences)|my (?:interests|preferences|hobbies))\b/i;
const PERSONAL_MEMORY_BLOCK = /\b(?:interests?|preferences?|hobbies|likes?|enjoys?|favourites?|favorites?|passions?)\b/i;
const MAX_MATCHED_BLOCKS_PER_SECTION = 2;

function words(value: string): Set<string> {
  const matched: string[] = value.toLowerCase().match(/[a-z0-9]+(?:-[a-z0-9]+)*/g) ?? [];
  return new Set(matched.filter((word) => word.length > 2 && !STOP_WORDS.has(word)));
}
...
function blocks(body: string): string[] {
  ...
  for (const line of body.split("\n")) {
    ...
    if (!line.trim() && current.length && !current[0]?.startsWith("- ")) flush();
...
export function selectRelevantMemorySummary(summary: string, prompt: string): string {
  ...
  for (let index = 0; index < matches.length; index++) {
    ...
    const candidates = blocks(body).map((block) => ({block, score: relevanceScore(block, query)}));
    ...
    const routeFloor = ROUTE_INTENT.test(prompt) && title === "What's in Memory" && strongest >= 3
      ? strongest - 1
      : 1;
    const selectedIndexes = new Set(candidates
      .map((candidate, candidateIndex) => ({...candidate, candidateIndex}))
      .filter(({block, score}) =>
        score >= routeFloor || (personalization && PERSONAL_MEMORY_BLOCK.test(block)),
      )
...
export function memorySummaryForPrompt(options: {
  ...
  return options.orchestrationExperiment
    ? selectRelevantMemorySummary(options.summary, options.prompt)
    : options.summary;
}
...
export function memorySummarySelectionForPrompt(options: {
  ...
  orchestrationExperiment: boolean;
  subagent: boolean;
}): { summary: string | undefined; candidateBlocks: number; retainedBlocks: number } {
  const selected = memorySummaryForPrompt(options);
  return {
    summary: selected,
    candidateBlocks: options.subagent ? 0 : memorySummaryBlockCount(options.summary),
