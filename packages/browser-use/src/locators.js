...
export async function findByLocator(send, locator) {
  ...
  if (locator.index >= outcome.count)
    throw new TargetError(
      `${described} matched ${outcome.count} element(s), so index ${locator.index} is out of range. Matched: ${outcome.describe.join(", ")}`,
    );
  if (outcome.count > 1 && !locator.index)
    // Ambiguity is reported, not silently resolved to the first hit — but the
    // first hit is still used, so a deliberate "the first Save button" works.
    void 0;

  const handle = await send("Runtime.evaluate", {
    expression: `window.__flareaiLocatorMatches[${locator.index}]`,
  });
