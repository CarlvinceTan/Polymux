...
QR/code linking, conversations, and an encrypted on-device Locker. Workspace
...
The Host owns every durable conversation and run. The phone app is a client;
it does not copy the agent runtime or expose Desktop workspace, browser, Drive,
Hub, Team management, or advanced settings.

## Install and check

Run these from the repository root. Phone is intentionally not a root npm
workspace, so install its dependencies explicitly.

```sh
npm --prefix apps/phone install
npm run phone:check
npm run phone:test
npm --prefix apps/phone run build
```

`npm run phone:dev` opens the Tauri desktop-sized development shell. Native
development uses:
...
Install the [official Tauri phone prerequisites](https://v2.tauri.app/start/prerequisites/)
...
2. In Polymux Phone, choose **Scan QR code** and scan the code under **Pair a
...
The Host connects outbound to Polymux Connect and the phone uses its HTTPS
...
Runs execute on the Personal Host, so they continue if the phone locks, loses
...
```sh
npm --prefix apps/phone run ios:build
npm --prefix apps/phone run android:build
```
...
For the hosted Supabase project, allow `polymux-phone://auth/callback` in Auth
redirect URLs and include `com.flarehq.polymux.phone` among Apple's accepted
client IDs. Keep existing desktop/web client IDs. `supabase/config.toml` includes
the phone redirect for local configuration; editing that file does not update
the hosted project. Public settings confirm Apple, Google, and email are enabled,
but native Apple audience and the hosted phone redirect still need verification.

## iOS Locker AutoFill

Locker already stores encrypted passwords, authenticator secrets and recovery
codes, and can unlock without a Host. Enable password and code AutoFill inside
the unlocked Locker, then enable Polymux in iOS Settings under AutoFill &
...
Local edits refresh the AutoFill copy. Foreground Locker sync detects remote
changes and refreshes it, or disables AutoFill if the vault cannot be unlocked.
Open and sync Locker after changes on another device: the extension does not
...
| --- | --- |
| `com.flarehq.polymux.phone` (Polymux) | Sign in with Apple, Push Notifications, AutoFill Credential Provider, App Groups |
| `com.flarehq.polymux.phone.autofill` (Polymux AutoFill) | AutoFill Credential Provider, App Groups |
| `group.com.flarehq.polymux.phone` | Shared group assigned to both identifiers |

Both targets also share Keychain group
`23YB4896XA.com.flarehq.polymux.phone.locker`. Team is `23YB4896XA`.
...
private key in the phone app. Debug uses APNs sandbox; release uses production.
The phone's notification control registers a token only after opt-in and refreshes
...
apps/phone/src-tauri/gen/apple/project.yml` from the repo root, not `ios:init`.
An unsigned simulator archive can be verified without provisioning:

```sh
npm --prefix apps/phone run ios:build -- --debug --target aarch64-sim --no-sign --ci --archive-only
swiftc apps/phone/src-tauri/plugins/apple/ios/Sources/AutoFillStore.swift apps/phone/src-tauri/apple/Tests/OneTimeCodeTests.swift -o /tmp/polymux-otp-tests
