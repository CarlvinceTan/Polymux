<script lang="ts">
  ...
  import LockerScreen from './LockerScreen.svelte';
  ...
  type AppTab = 'assistant' | 'team' | 'hub' | 'locker' | 'settings';
  ...
  const appBarIconMetrics = {
    ...
    locker: {viewBox: '0 0 24 24', strokeWidth: 1.5},
  ...
  export let lockerCall: (method: string, args?: JsonValue[]) => Promise<unknown>;
...
<section class="chat-list-pane" aria-labelledby="chats-title">
  <header class="chat-list-header">
    ...
    {/if}
    {#if activeTab === 'settings' || activeTab === 'locker'}
      <span class="header-spacer" aria-hidden="true"></span>
    {:else}
  ...
  {#if activeTab === 'locker'}
    <LockerScreen {online} call={lockerCall} />
  {:else if activeTab === 'settings'}
    <section class="settings-view" aria-label="Settings">
      ...
      <button class="disconnect-button" type="button" on:click={onDisconnect}>Disconnect this phone</button>
    </section>
  ...
  <nav class="app-bar" aria-label="Primary">
    ...
    <button class:active={activeTab === 'locker'} class="app-tab" type="button" on:click={() => selectTab('locker')} aria-current={activeTab === 'locker' ? 'page' : undefined}><Icon name="key" size={21} {...appBarIconMetrics.locker} /><span>Locker</span></button>
