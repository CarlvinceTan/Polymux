...
const LOCKER_ORIGIN = "http://127.0.0.1:47654";
const CACHE_KEY = "polymux-locker-vault";
...
function lockerUrl(path, query) {
  const url = new URL(path, LOCKER_ORIGIN);
  if (url.origin !== LOCKER_ORIGIN || !url.pathname.startsWith("/v1/locker/"))
    throw new Error("Invalid Locker endpoint");
  if (query) {
...
function desktopOffline() {
  return Object.assign(new Error("Open Polymux to use Locker"), {code: DESKTOP_OFFLINE});
}

export function desktopLockerUnavailableReason() {
  return chrome.runtime.getURL("").startsWith("safari-web-extension:")
    ? "Safari uses the vault saved in this browser or your account vault. Desktop Locker connection is not available."
    : "";
}

let desktopCapability = null;
async function lockerCapability() {
  if (desktopLockerUnavailableReason()) throw desktopOffline();
  if (!desktopCapability) {
    desktopCapability = chrome.runtime.sendNativeMessage("com.polymux.tab_context", {type: "polymux:locker-connection"})
      .then((reply) => {
...
export async function lockerRequest(path, options = {}, retry = true) {
  const url = lockerUrl(path, options.query);
  let response;
  try {
    const capability = await lockerCapability();
  ...
  if (response.status === 403) {
    ...
    if (retry) return lockerRequest(path, options, false);
    throw Object.assign(new Error("Reconnect the Polymux browser host to use desktop Locker"), {code: "client_authentication"});
  }
  let value = {};
  try {
  ...
  if (!response.ok)
    throw Object.assign(new Error(value.error || `Locker returned ${response.status}`), {
...
export async function syncCachedVault() {
  try {
    const blob = await lockerRequest("/v1/locker/export");
...
export async function pushCachedVault() {
  ...
  await lockerRequest("/v1/locker/import", {method: "POST", body: blob});
...
export async function accountCloud() {
  ...
  return new FetchLockerCloud(account);
}

export function createDeviceSession() {
  return new LockerSession({
    async load() {
      const current = parseLockerVaultBlob(await loadCachedVault());
...
async function webauthnDesktop(payload) {
  ...
  if (payload.action === "status") {
    const status = await lockerRequest("/v1/locker/status");
    return {unlocked: Boolean(status.unlocked), exists: Boolean(status.exists), locked: !status.unlocked};
  }
  if (payload.action === "offers") {
    const value = await lockerRequest("/v1/locker/passkeys", {method: "POST", body: request});
    return {offers: value.offers ?? []};
  }
  if (payload.action === "get") {
    const value = await lockerRequest("/v1/locker/passkey/get", {method: "POST", body: request});
    return {credential: value};
  }
  if (payload.action === "create") {
    const value = await lockerRequest("/v1/locker/passkey/create", {method: "POST", body: request});
    return {credential: value};
  }
...
async function webauthnDevice(payload) {
  ...
  if (!session.status().unlocked) return {locked: true, error: "Locker is locked"};
  if (payload.action === "offers") {
