import {
  ...
  canOAuth,
  createDeviceSession,
  desktopLockerUnavailableReason,
  getAccount,
  lockerRequest,
...
function page(html) {
  const availability = desktopLockerUnavailableReason();
...
async function tabPage(tabId) {
  ...
  try {
    return await chrome.tabs.sendMessage(tabId, {type: "polymux:locker-page-query"});
  } catch {
    return null;
  }
}
...
async function fillTab(tabId, fields) {
  ...
  const result = await chrome.tabs.sendMessage(tabId, {type: "polymux:locker-fill", ...fields});
...
async function render() {
  ...
  try {
    await preferDesktopLoopback(
      async () => {
        const locker = await lockerRequest("/v1/locker/status");
        try {
        ...
        if (!locker.exists) {
          page(`<h1>Locker</h1><p class="status">Create a locker in Polymux first.</p>${accountStrip(status)}`);
          bindAccount(status);
          return "desktop";
        }
        if (!locker.unlocked) {
          unlockForm("Locker", "", status);
          bindUnlock(async (password) => {
            await lockerRequest("/v1/locker/unlock", {method: "POST", body: {password}});
            await syncCachedVault();
          });
          return "desktop";
        }
        await renderMatches(url, tab, status, {
          async matches(pageUrl) {
            return (await lockerRequest("/v1/locker/matches", {query: {url: pageUrl}})).items ?? [];
          },
          async fill(id) {
            return lockerRequest("/v1/locker/fill", {method: "POST", body: {id}});
          },
          async save(body) {
            await lockerRequest("/v1/locker/save", {method: "POST", body});
            await syncCachedVault();
          },
          async lock() {
            await lockerRequest("/v1/locker/lock", {method: "POST", body: {}});
          },
        });
        return "desktop";
      },
    ...
    page(`<h1>Locker</h1><p class="status">${escape(cause instanceof Error ? cause.message : String(cause))}</p>${accountStrip(status)}`);
    bindAccount(status);
  }
}
...
async function renderDevice(url, tab, status) {
  ...
  if (!cached.status().exists) {
    page(`
      <h1>Locker</h1>
      <p class="status">${status.signedIn ? "This account has no locker in the cloud." : desktopLockerUnavailableReason() ? "Sign in to use the account locker." : "Connect the Polymux browser host, or sign in to use the account locker."}</p>
      ${accountStrip(status)}
    `);
    bindAccount(status);
    return;
  ...
  if (!cached.status().unlocked) {
    unlockForm("Locker", status.signedIn ? "" : "Unlocking the copy saved on this browser.", status);
    bindUnlock((password) => cached.unlock(password).then(async () => {
    await chrome.runtime.sendMessage({type: "polymux:locker-device-unlock", password});
  }));
    return;
  }
  await renderMatches(url, tab, status, {
...
async function renderMatches(url, tab, status, api) {
  ...
  const pending = await chrome.runtime.sendMessage({type: "polymux:locker-pending"});
  const rows = matches
    .map(
      (item) => `
  ...
  document.getElementById("save")?.addEventListener("click", async () => {
    ...
    await chrome.runtime.sendMessage({type: "polymux:locker-clear-pending"});
    await render();
  });
