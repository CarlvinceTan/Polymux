// Vault form detection and fill. Same heuristics as the in-app Browser view.
(() => {
  "use strict";
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type === "polymux:vault-fill") {
      sendResponse({ok: fill(message)});
      return false;
    }
    if (message?.type === "polymux:vault-page-query") {
      const forms = loginForms();
      sendResponse({
        origin: location.origin,
        url: location.href,
