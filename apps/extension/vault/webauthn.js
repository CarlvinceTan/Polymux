 ...
 * scripts never see vault titles until the user picks one.
 */
(() => {
  "use strict";
  ...
  const UNLOCK_HINT = chrome.runtime.getURL("").startsWith("safari-web-extension:")
    ? "Unlock Vault in the extension popup, then retry."
    : "Unlock Vault in Polymux or the extension popup, then retry.";
  let current = null;

  function injectPageScript() {
    if (document.documentElement?.querySelector("script[data-polymux-webauthn]")) return;
    const script = document.createElement("script");
    script.src = chrome.runtime.getURL("vault/webauthn-page.js");
    script.async = false;
    ...
    return new Promise((resolve) => {
      ...
      retry?.addEventListener("click", async () => {
        ...
        try {
          ...
          if (!next?.unlocked) throw new Error(`Vault is still locked. ${UNLOCK_HINT}`);
          closeOverlay();
          resolve(true);
        } catch (cause) {
          if (error) {
