...
/**
 * Polymux's window-control pill — the menu-bar presence of the agent driving a
 * native window.
 *
...
export interface WindowControlMenubarOptions {
...
export class WindowControlMenubar {
  readonly #options: WindowControlMenubarOptions;
  #tray: Tray | null = null;
  #shown = "";
  /** Set by `hide()` so an in-flight redraw cannot resurrect the pill. */
  #released = false;

  constructor(options: WindowControlMenubarOptions) {
