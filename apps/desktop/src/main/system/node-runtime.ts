...
/**
 * The interpreter behind `"${FLAREAI_NODE:-node}"` in the skill scripts.
 *
 ...
 *    interpreter with FlareAI's TCC grants.
 ...
 *  - Neither (a packaged app whose runtime went missing): FLAREAI_NODE stays
 ...
 * "FlareAI" for the Dock (scripts/dev-app-name.mjs), which makes a dev run
...
export function resolveNodeRuntime(options: {
  ...
  const wrapper = path.join(options.wrapperDirectory, "flareai-node");
  mkdirSync(options.wrapperDirectory, { recursive: true });
  writeFileSync(
    wrapper,
    `#!/bin/sh\n# Written by FlareAI at launch: runs the dev Electron as Node.\nELECTRON_RUN_AS_NODE=1 exec "${options.execPath}" "$@"\n`,
  );
...
/**
 * Resolves the runtime and exports it as FLAREAI_NODE, which reaches the
...
export function exportNodeRuntime(options: {
  ...
  if (runtime) process.env.FLAREAI_NODE = runtime;
  return runtime;
}
