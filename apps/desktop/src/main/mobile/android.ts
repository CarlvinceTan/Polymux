...
import type {MobileDeviceDto, MobileFrameDto, MobilePointDto, MobileStatusDto} from "@polymux/protocol";
...
export interface AndroidMobileControllerOptions {
 ...
 * MobileView a portable fallback while the smoother scrcpy stream is optional. */
export class AndroidMobileController {
  readonly #platform: NodeJS.Platform;
  readonly #resourcesDirectory: string;
  readonly #adbOverride: string | null;
  #activeDeviceId: string | null = null;

  constructor(options: AndroidMobileControllerOptions = {}) {
  ...
  async status(): Promise<MobileStatusDto> {
    ...
    if (devices.length > 1) return statusDto(null, "error", "More than one Android mobile is connected. Disconnect the others, then check again.");
    ...
    if (device.pairingState !== "paired")
      return statusDto(device, "error", "Approve USB debugging on the Android mobile, then check again.");
  ...
  async connect(): Promise<MobileStatusDto> {
    ...
    const devices = await this.#devices(adb);
    if (devices.length > 1) throw new Error("More than one Android mobile is connected. Disconnect the others, then try again.");
    const device = devices[0];
    if (!device) throw new Error("Connect the Android mobile with USB, or pair it from Wireless debugging.");
    if (device.pairingState !== "paired") throw new Error("Approve USB debugging on the Android mobile first.");
  ...
  async pair(pairingAddress: string, pairingCode: string, connectAddress?: string): Promise<MobileStatusDto> {
    const adb = this.#adb();
    if (!adb) throw new Error("This Polymux build does not include Android platform tools.");
  ...
  async #connectedDevice(): Promise<{adb: string; device: MobileDeviceDto}> {
    const adb = this.#adb();
    ...
    if (!device || device.pairingState !== "paired") throw new Error("Connect the Android mobile in Mobile before controlling it.");
    return {adb, device};
  }

  async #devices(adb: string): Promise<MobileDeviceDto[]> {
    ...
    return [...grouped.entries()].map(([hardwareSerial, candidates]): MobileDeviceDto => {
      // During guided wireless setup ADB intentionally lists the same physical
      // mobile twice (USB serial + host:port). Prefer the authorized Wi-Fi route
      ...
      const model = route.candidate.model || "Android mobile";
      return {
        platform: "android",
  ...
  #adb(): string | null {
    ...
    const candidates = [
      ...
      path.join(this.#resourcesDirectory, "mobile", "android", executable),
      path.join(this.#resourcesDirectory, "resources", "mobile", "android", executable),
...
export function adbTransport(serial: string): MobileDeviceDto["transport"] {
  return /:\d+$/.test(serial) ? "wireless" : "wired";
}

function statusDto(device: MobileDeviceDto | null, stage: MobileStatusDto["stage"], message: string | null): MobileStatusDto {
...
function coordinate(value: number): string {
  if (!Number.isFinite(value) || value < 0) throw new Error("Mobile coordinates must be positive numbers.");
