...
import type {MobileIosSigningStatusDto} from "@polymux/protocol";
...
export interface IosMobileSignerClient {
  status(): Promise<MobileIosSigningStatusDto>;
  beginLogin(email: string, password: string): Promise<MobileIosSigningStatusDto>;
  completeTwoFactor(code: string): Promise<MobileIosSigningStatusDto>;
  logout(): Promise<MobileIosSigningStatusDto>;
  signWda(input: {
    source: string;
    output: string;
    udid: string;
...
export interface IosMobileSignerOptions {
...
export class IosMobileSigner implements IosMobileSignerClient {
  ...
  #sequence = 0;
  #verificationMethod: MobileIosSigningStatusDto["verificationMethod"] = null;
  #stderr = "";

  constructor(options: IosMobileSignerOptions) {
  ...
  async status(): Promise<MobileIosSigningStatusDto> {
  ...
  async beginLogin(email: string, password: string): Promise<MobileIosSigningStatusDto> {
  ...
  async completeTwoFactor(code: string): Promise<MobileIosSigningStatusDto> {
  ...
  async logout(): Promise<MobileIosSigningStatusDto> {
  ...
  async #request(method: string, params: Record<string, unknown> = {}, timeoutMs = 45_000): Promise<unknown> {
    ...
    const id = `mobile-signer-${++this.#sequence}`;
  ...
  async #ensureProcess(): Promise<ChildProcessWithoutNullStreams> {
    ...
    const runtime = this.#runtime();
    if (!runtime) throw new Error("This Polymux build does not include local iPhone signing.");
    const stateDirectory = path.join(this.#dataDirectory, "mobile", "ios", "signer-state");
  ...
  #runtime(): {python: string; helper: string; zsign: string} | null {
    ...
    const directories = [
      ...
      path.join(this.#resourcesDirectory, "mobile", "ios", "signer"),
      path.join(this.#resourcesDirectory, "resources", "mobile", "ios", "signer"),
...
function unavailableStatus(): MobileIosSigningStatusDto {
  return {
    supported: false,
    stage: "unavailable",
    email: null,
