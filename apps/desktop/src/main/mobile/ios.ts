...
import {IosMobileSigner, type IosMobileSignerClient} from "./ios-signer.js";
...
export interface GoIosDevice {
  ...
  transport: MobileDeviceDto["transport"];
}
...
export interface IosMobileControllerOptions {
 ...
 */
export class IosMobileController {
  ...
  readonly #signer: IosMobileSignerClient;
  ...
  #activeDevice: MobileDeviceDto | null = null;
  #recentDevice: MobileDeviceDto | null = null;
  #recentDeviceAt = 0;

  constructor(options: IosMobileControllerOptions) {
    ...
    this.#signer = options.signer ?? new IosMobileSigner({
      ...
      message: !device
        ? ambiguous
          ...
          : selection.activeMissing
            ? "The iPhone connected to Mobile is unavailable. Reconnect it, or press Stop before choosing another iPhone."
            : disconnectedMessage
        : !signing.available
          ? signing.message
          : running
  ...
  async #connect(): Promise<MobileStatusDto> {
    ...
    if (!device) {
      if (selection.activeMissing)
        throw new Error("Reconnect the iPhone already connected to Mobile, or press Stop before choosing another iPhone.");
  ...
  async #stop(): Promise<MobileStatusDto> {
    ...
    this.#runner = null;
    const devices = await this.#devices().catch((): MobileDeviceDto[] => []);
    ...
    if (device) {
      ...
      throw new Error("WebDriverAgent is still running on the iPhone. Try Stop mobile control again.");
    }
    await this.#stopProxy();
    await this.#stopGoIosTunnel();
    this.#activeDevice = null;
  ...
  async swipe(from: MobilePointDto, to: MobilePointDto, durationMs = 350): Promise<void> {
    const {baseUrl: url} = await this.#connectedDevice();
  ...
  async type(text: string): Promise<void> {
    if (!text) return;
    if (text.length > 4_000) throw new Error("Mobile text is limited to 4,000 characters.");
    const {baseUrl: url} = await this.#connectedDevice();
  ...
  async #devices(): Promise<MobileDeviceDto[]> {
    ...
    const enumerated = goIos
      ...
      : helper
        ...
        : [];
    const wireless = await this.#deviceHelperWirelessDevices().catch((): MobileDeviceDto[] => []);
  ...
  #rememberDevices(devices: MobileDeviceDto[]): MobileDeviceDto[] {
    if (devices.length === 1) {
  ...
  async #deviceHelperWirelessDevices(): Promise<MobileDeviceDto[]> {
    const runtime = this.#deviceHelper();
    if (!runtime) return [];
    const pairingDirectory = path.join(this.#dataDirectory, "mobile", "ios", "remote-pairing");
  ...
  async #goIosDevices(goIos: string): Promise<MobileDeviceDto[]> {
    ...
    return Promise.all(result.map(async (candidate): Promise<MobileDeviceDto> => {
  ...
  async #helperDevices(helper: string): Promise<MobileDeviceDto[]> {
  ...
  #iosHelper(): string | null {
    ...
    const candidates = [
      ...
      path.join(this.#resourcesDirectory, "mobile", "ios", executable),
      path.join(this.#resourcesDirectory, "resources", "mobile", "ios", executable),
    ];
  ...
  #deviceHelper(): {python: string; helper: string} | null {
    ...
    const helperCandidates = [
      ...
      path.join(this.#resourcesDirectory, "mobile", "ios", "device", "helper.py"),
      path.join(this.#resourcesDirectory, "resources", "mobile", "ios", "device", "helper.py"),
    ];
    const pythonCandidates = [
      this.#deviceHelperPythonOverride,
      process.env.POLYMUX_IOS_DEVICE_PYTHON_PATH,
      path.join(this.#resourcesDirectory, "mobile", "ios", "device", "python", this.#platform === "win32" ? "" : "bin", pythonExecutable),
      path.join(this.#resourcesDirectory, "resources", "mobile", "ios", "device", "python", this.#platform === "win32" ? "" : "bin", pythonExecutable),
      path.join(this.#resourcesDirectory, "mobile", "ios", "signer", "python", this.#platform === "win32" ? "" : "bin", pythonExecutable),
      path.join(this.#resourcesDirectory, "resources", "mobile", "ios", "signer", "python", this.#platform === "win32" ? "" : "bin", pythonExecutable),
    ];
  ...
  #goIos(): string | null {
    ...
    const candidates = [
      ...
      path.join(this.#resourcesDirectory, "mobile", "ios", "tools", executable),
      path.join(this.#resourcesDirectory, "resources", "mobile", "ios", "tools", executable),
      path.join(this.#resourcesDirectory, "mobile", "ios", executable),
      path.join(this.#resourcesDirectory, "resources", "mobile", "ios", executable),
    ];
    return candidates.find((candidate): candidate is string => Boolean(candidate && existsSync(candidate))) ?? null;
  ...
  #sourceApp(): string | null {
    const candidates = [
      ...
      this.#preparedApp(),
      path.join(this.#dataDirectory, "mobile", "ios", "wda-runtime", "DerivedData", "Build", "Products", "Debug-iphoneos", RUNNER_APP),
      path.join(this.#resourcesDirectory, "mobile", "ios", RUNNER_APP),
      path.join(this.#resourcesDirectory, "resources", "mobile", "ios", RUNNER_APP),
    ];
    return candidates.find((candidate): candidate is string => Boolean(candidate && existsSync(candidate))) ?? null;
  }
  ...
  #preparedApp(): string {
    return path.join(this.#dataDirectory, "mobile", "ios", "prepared", RUNNER_APP);
  }

  #installationReceiptPath(): string {
    return path.join(this.#dataDirectory, "mobile", "ios", "installation.json");
  }

  async #installationCurrent(source: string, device: MobileDeviceDto, bundleId: string): Promise<boolean> {
  ...
  async #writeInstallationReceipt(source: string, device: MobileDeviceDto, bundleId: string): Promise<void> {
  ...
  async #signingStatus(source: string, device: MobileDeviceDto): Promise<MobileStatusDto["signing"]> {
  ...
  async #prepare(source: string, device: MobileDeviceDto): Promise<string> {
    ...
    const scratch = await mkdtemp(path.join(tmpdir(), "polymux-mobile-sign-"));
    try {
  ...
  async #installApp(device: MobileDeviceDto, appPath: string): Promise<void> {
  ...
  async #start(device: MobileDeviceDto, bundleId: string, force = false): Promise<void> {
    const deviceHelper = this.#deviceHelper();
  ...
  async #connectionUrl(device: MobileDeviceDto, startProxy = true): Promise<string | null> {
  ...
  #existingConnectionUrl(device: MobileDeviceDto): string | null {
  ...
  #selectEnumeratedDevice(devices: MobileDeviceDto[]): {
  ...
  async #retainedActiveDevice(): Promise<MobileDeviceDto | null> {
  ...
  async #isRunning(device: MobileDeviceDto, startProxy = true): Promise<boolean> {
    const url = await this.#connectionUrl(device, startProxy);
    return url ? probeWda(url) : false;
  }

  async #ensureProxy(device: MobileDeviceDto): Promise<string> {
    const goIos = this.#goIos();
  ...
  async #ensureGoIosForward(goIos: string, device: MobileDeviceDto): Promise<string> {
  ...
  async #ensureGoIosTunnel(goIos: string, device: MobileDeviceDto): Promise<void> {
    ...
    const pairingDirectory = path.join(this.#dataDirectory, "mobile", "ios", "go-ios-pairing");
  ...
  async #goIosTunnelReady(goIos: string, device: MobileDeviceDto): Promise<boolean> {
  ...
  #goIosArgs(device: MobileDeviceDto, command: string[]): string[] {
  ...
  async #restartProxy(device: MobileDeviceDto): Promise<string | null> {
  ...
  async #isInstalled(device: MobileDeviceDto): Promise<boolean> {
    const source = this.#sourceApp();
  ...
  async #terminateProcess(device: MobileDeviceDto, executable: string, bundleId: string): Promise<void> {
    if (usesCoreDeviceTransport(this.#platform, device.id)) {
      const result = await deviceCtlJson([
  ...
  async #connectedDevice(): Promise<{device: MobileDeviceDto; baseUrl: string}> {
    const devices = await this.#devices();
    const selection = this.#selectEnumeratedDevice(devices);
    if (!this.#activeDevice && devices.length > 1)
      throw new Error("More than one iPhone is connected. Disconnect the others before controlling Mobile.");
    ...
    if (!device) {
      if (selection.activeMissing)
        throw new Error("The iPhone connected to Mobile is unavailable. Reconnect it, or press Stop before choosing another iPhone.");
    ...
    if (!await probeWda(url)) {
      ...
      if (!await probeWda(url)) {
        ...
        if (!bundleId || (!this.#deviceHelper() && !await this.#isInstalled(device).catch(() => false)))
          throw new Error("Connect the iPhone in Mobile before controlling it.");
  ...
  async #withReadRecovery<T>(
    operation: (connection: {device: MobileDeviceDto; baseUrl: string}) => Promise<T>,
  ): Promise<T> {
    try {
  ...
  /** Keep one XCTest session alive for every MobileView and agent client. If
...
export function parseCoreDevices(value: unknown): MobileDeviceDto[] {
...
export function parseDeviceHelperWireless(output: string): MobileDeviceDto[] {
  let value: unknown;
  try {
  ...
  return devices.flatMap((candidate): MobileDeviceDto[] => {
    if (!candidate || typeof candidate !== "object") return [];
 ...
 * network records emitted during a guided wireless handoff into one mobile.
...
export function pymobileXcuiTestArgs(
  platform: NodeJS.Platform,
  device: Pick<MobileDeviceDto, "udid" | "transport">,
  bundleId: string,
): string[] {
...
async function deviceCtlJson(args: string[]): Promise<unknown> {
  const scratch = await mkdtemp(path.join(tmpdir(), "polymux-mobile-device-"));
  try {
...
): {device: MobileDeviceDto | null; activeMissing: boolean; ambiguous: boolean} {
  if (activeUdid) {
...
function missingSigning(message: string): MobileStatusDto["signing"] {
  return {available: false, source: "none", expiresAt: null, teamId: null, message};
}

function unsupportedStatus(): MobileStatusDto {
  return {
    supported: false,
    stage: "unsupported",
    device: null,
