...
import {AndroidMobileController, adbTransport, parseAdbConnectAddress, parseAdbDevices, pngDimensions} from "./android.js";
...
`, "utf8");
  ...
  try {
    const controller = new AndroidMobileController({platform: "darwin", adbPath: adb});
...
test("groups USB and wireless routes for one mobile and prefers wireless", async () => {
...
`, "utf8");
  await chmod(adb, 0o755);
  try {
    const controller = new AndroidMobileController({platform: "win32", adbPath: adb});
