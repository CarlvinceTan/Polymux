...
import type {MobileDeviceDto} from "@polymux/protocol";
import {
...
const firstPhone: MobileDeviceDto = {
...
test("groups go-ios USB and network routes as one physical iPhone", () => {
  const devices = parseGoIosDevices(JSON.stringify({deviceList: [
    {Udid: "mobile-udid", ProductType: "iPhone17,1", ProductVersion: "26.6", ConnectionType: "USB"},
    {Udid: "mobile-udid", ProductType: "iPhone17,1", ProductVersion: "26.6", ConnectionType: "Network"},
  ]}));
...
test("keeps a running userspace tunnel discoverable after USB enumeration disappears", () => {
  const devices = parseGoIosDevices(JSON.stringify({deviceList: [
    {Udid: "mobile-udid", ProductType: "", ProductVersion: "", ConnectionType: "userspaceTunnel"},
  ]}));
  assert.deepEqual(devices, [{
    udid: "mobile-udid",
    productType: "",
  ...
  const devices = parseCoreDevices([
    {
      ...
      hardwareProperties: {marketingName: "iPhone 16 Pro", platform: "iOS", reality: "physical", udid: "mobile-udid"},
    },
    {
...
test("keeps CoreDevice operations on the backend that discovered the iPhone", () => {
  ...
  assert.equal(usesCoreDeviceTransport("darwin", "ios:mobile-udid"), false);
...
test("reads WDA bundle metadata without macOS PlistBuddy", () => {
  const info = `<?xml version="1.0"?><plist><dict>
    <key>CFBundleIdentifier</key><string>com.example.mobile&amp;control</string>
    <key>CFBundleExecutable</key><string>MobileRunner</string>
  </dict></plist>`;
  assert.equal(parseInfoPlistString(info, "CFBundleIdentifier"), "com.example.mobile&control");
  assert.equal(parseInfoPlistString(info, "CFBundleExecutable"), "MobileRunner");
...
test("uses the no-Xcode XCUITest launcher on every desktop platform", () => {
  const wired = {udid: "mobile-udid", transport: "wired" as const};
  assert.deepEqual(pymobileXcuiTestArgs("darwin", wired, "test.runner"), [
    "developer", "dvt", "xcuitest", "--native",
    "--udid", "mobile-udid", "--env", "USE_PORT=8100", "test.runner",
  ]);
