...
import type {MobileStatusDto} from "@polymux/protocol";
import {createMobileTool, type MobileAutomation} from "./tools.js";

const status: MobileStatusDto = {
  supported: true,
...
function automation(actions: unknown[]): MobileAutomation {
  return {
    status: async () => status,
    connect: async () => status,
    frame: async () => ({deviceId: "mobile", dataUrl: "data:image/png;base64,AAAA", width: 402, height: 874, capturedAt: "now"}),
...
test("keeps action-specific arguments optional without advertising a strict schema", () => {
  const tool = createMobileTool(automation([]));
  assert.equal(tool.strict, undefined);
  assert.deepEqual(tool.parameters.required, ["action"]);
});

test("returns a screenshot as a model-visible image", async () => {
  const result = await createMobileTool(automation([])).execute({action: "screenshot"}, context);
...
test("redacts stable device and signing identifiers from agent status", async () => {
  const result = await createMobileTool(automation([])).execute({action: "status"}, context);
...
test("forwards tap and swipe coordinates without guessing defaults", async () => {
  ...
  const tool = createMobileTool(automation(actions));
