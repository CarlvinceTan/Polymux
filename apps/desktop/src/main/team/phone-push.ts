...
export interface PhonePushSubscription { token: string; environment: 'sandbox' | 'production' }
export function parsePhonePush(value: unknown): PhonePushSubscription | null {
...
export function phonePushAvailable(): boolean {
  return Boolean(process.env.POLYMUX_APNS_KEY_PATH && process.env.POLYMUX_APNS_KEY_ID && process.env.POLYMUX_APNS_TEAM_ID);
}

/** Server-side APNs credentials stay on the Host; the phone receives no signing key. */
export async function sendPhonePush(subscription: PhonePushSubscription): Promise<'sent' | 'expired'> {
  const {POLYMUX_APNS_KEY_PATH: path, POLYMUX_APNS_KEY_ID: kid, POLYMUX_APNS_TEAM_ID: team} = process.env;
  if (!path || !kid || !team) throw new Error('Phone notifications are not configured on this Host.');
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = client.request({
      ...
      'apns-topic': 'com.flarehq.polymux.phone', 'apns-push-type': 'alert', 'apns-priority': '10',
