...
export async function getAllProviders(): Promise<Provider[]> {
  ...
  if (zc) all.push(zc)
  if (zd) all.push(zd)
  return all.filter(provider => !provider.network)
}
...
export function discoveryFailures(): string[] { return [...warnedDiscoveryFailures] }

async function discoverOne(provider: Provider): Promise<{ sources: SessionSource[]; failed: boolean }> {
  try {
    const excluded = JSON.parse(process.env.POLYMUX_USAGE_EXCLUDED_ROOTS || '[]') as string[]
    const canonical = (value: string) => { try { return realpathSync(value) } catch { return path.resolve(value) } }
    const roots = excluded.map(canonical)
    const sources = (await provider.discoverSessions()).filter(source => {
      const location = canonical(source.path)
      return !roots.some(root => location === root || location.startsWith(root + path.sep))
    })
