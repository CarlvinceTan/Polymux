...
export type { VaultCloudBlob, VaultCloudStore };
export { vaultChecksum };

/** Physical bucket and table names predate the vault rename and stay as-is:
 * they live server-side, where renaming would orphan every existing account
 * copy. See supabase/migrations/20260907132400_locker_vault.sql. */
const BUCKET = "locker";
const TABLE = "locker_vaults";

/** Encrypted kdbx only. Metadata rows never hold secrets. */
export class SupabaseVaultCloud implements VaultCloudStore {
  ...
  async pull(): Promise<VaultCloudBlob | null> {
    ...
    if (error || !data)
      throw new Error(
        cloudError(error?.message ?? "The vault could not be downloaded."),
      );
  ...
  async push(blob: VaultCloudBlob): Promise<void> {
    const client = this.#account.client;
    const userId = this.#account.status().profile?.userId;
    if (!client || !userId) throw new Error("Sign in to sync this vault.");
    ...
    const { error: rowError } = await client.from(TABLE).upsert({
      user_id: userId,
