...
test("ACP registry package distributions become launch presets", () => {
  assert.deepEqual(parseAcpRegistry({agents: [
    ...
    {id: "codex-acp", name: "Codex", version: "1.2.3", description: "Adapter", icon: "https://example.test/codex.svg", command: "npx", args: ["-y", "@acp/codex@1.2.3", "--quiet"]},
    {id: "python", name: "Python agent", version: "", description: "", icon: "", command: "uvx", args: ["python-agent", "serve"]},
    {id: "binary", name: "Binary only", version: "", description: "", icon: "", command: "", args: []},
  ]);
});
