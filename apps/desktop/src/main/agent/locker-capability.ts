...
export function loadLockerCapability(file: string): string {
  ...
  try {
    ...
    if (!stat.isFile() || (process.platform !== "win32" &&
      ...
      throw new Error("Locker extension capability must be a private file owned by this user");
...
export function matchesLockerCapability(header: string | undefined, token: string): boolean {
