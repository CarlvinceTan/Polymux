...
import {loadVaultCapability} from "./vault-capability.js";
...
async function withServer(run: (server: AgentSurfaceServer, base: string, authorizedFetch: typeof fetch) => Promise<void>): Promise<void> {
  ...
  const vaultCapabilityPath = join(directory, "capability");
  const token = loadVaultCapability(vaultCapabilityPath);
  ...
  const server = new AgentSurfaceServer({port, vaultCapabilityPath});
  await server.start();
  try {
...
test("explicit extension negotiation reports the independent extension version", async () => {
  await withServer(async (_server, base) => {
    ...
    assert.deepEqual(snapshot.surface.capabilities, ["surface-feed-v1", "vault-fill-v1"]);
  });
});
...
test("vault fill stays on the loopback until the user asks", async () => {
  await withServer(async (server, base, authorizedFetch) => {
    const missing = await authorizedFetch(`${base}/v1/vault/status`);
    assert.equal(missing.status, 503);
    let unlocked = false;
    const items = [{id: "gh", title: "GitHub", username: "ada", url: "https://github.com", notes: "", groupId: "g", groupName: "Vault", hasPassword: true, hasTotp: true, hasRecoveryCodes: false, hasPasskey: false, updatedAt: null as string | null}];
    server.attachVault({
      status: () => ({
        exists: true,
        unlocked,
      ...
      fill: (id) => {
        ...
        if (id !== "gh") throw new Error("That item is not in the vault");
    ...
    for (const path of Object.values(VAULT_SURFACE_PATHS)) {
    ...
    const locked = await (await authorizedFetch(`${base}/v1/vault/status`)).json();
    assert.equal(locked.unlocked, false);
    assert.deepEqual((await (await authorizedFetch(`${base}/v1/vault/matches?url=https://github.com/login`)).json()).items, []);
    const unlockedStatus = await authorizedFetch(`${base}/v1/vault/unlock`, {
      method: "POST",
      headers: {"content-type": "application/json"},
      body: JSON.stringify({password: "secret-password"}),
    });
    assert.equal(unlockedStatus.ok, true);
    const matches = await (await authorizedFetch(`${base}/v1/vault/matches?url=https://github.com/login`)).json();
    assert.equal(matches.items[0].id, "gh");
    const fill = await (await authorizedFetch(`${base}/v1/vault/fill`, {
      method: "POST",
      headers: {"content-type": "application/json"},
      body: JSON.stringify({id: "gh"}),
    })).json();
    assert.deepEqual(fill, {username: "ada", password: "s3cret", totp: "123456"});
    const exported = await (await authorizedFetch(`${base}/v1/vault/export`)).json();
...
test("Vault capability does not authorize website origins, spoofed hosts or public preflights", async () => {
  const directory = mkdtempSync(join(tmpdir(), "polymux-vault-auth-"));
  const vaultCapabilityPath = join(directory, "capability");
  const token = loadVaultCapability(vaultCapabilityPath);
  ...
  try {
    for (const headers of [
      ...
      const response = await fetch(`http://127.0.0.1:${port}/v1/vault/status`, {headers});
      assert.equal(response.status, 403);
      assert.equal(response.headers.get("Access-Control-Allow-Origin"), null);
    }
    await new Promise<void>((resolve, reject) => {
      const request = httpRequest(`http://127.0.0.1:${port}/v1/vault/status`, {
    ...
    const preflight = await fetch(`http://127.0.0.1:${port}/v1/vault/fill`, {
      method: "OPTIONS", headers: {Origin: "https://attacker.example", "Access-Control-Request-Headers": "authorization"},
    });
    assert.equal(preflight.status, 403);
    assert.equal(preflight.headers.get("Access-Control-Allow-Origin"), null);
    const noOrigin = await fetch(`http://127.0.0.1:${port}/v1/vault/status`, {headers: {Authorization: `Bearer ${token}`}});
    assert.equal(noOrigin.status, 503); // Authenticated; no Vault attached in this fixture.
    assert.equal(noOrigin.headers.get("Access-Control-Allow-Origin"), null);
    const extensionOrigin = `chrome-extension://${"a".repeat(32)}`;
    const approved = await fetch(`http://127.0.0.1:${port}/v1/vault/status`, {headers: {Authorization: `Bearer ${token}`, Origin: extensionOrigin}});
    ...
    const firefox = await fetch(`http://127.0.0.1:${port}/v1/vault/status`, {headers: {Authorization: `Bearer ${token}`, Origin: firefoxOrigin}});
    assert.equal(firefox.status, 503);
    assert.equal(firefox.headers.get("Access-Control-Allow-Origin"), firefoxOrigin);
    const unapprovedFirefox = await fetch(`http://127.0.0.1:${port}/v1/vault/status`, {headers: {Origin: firefoxOrigin}});
