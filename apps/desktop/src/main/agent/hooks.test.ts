...
test("a hook that never reads the payload still reports its exit code", async () => {
  // The payload is deliberately larger than a pipe buffer, so the write cannot
  // land in one go and the child's exit closes the pipe underneath it. That
  // EPIPE used to escape as an unhandled stream error and fail the call.
  await withConfig(
    {hooks: [{event: "pre-tool", tools: ["write"], command: "exit 0"}]},
    async (engine) => {
      const decision = await engine.beforeTool(call("write", {body: "x".repeat(1_000_000)}));
      assert.equal(decision.allow, true);
    },
  );
});
