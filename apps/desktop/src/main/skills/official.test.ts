...
function bundle(): {source: string; home: string} {
  ...
  const script = path.join(source, "computer-use", "scripts", "run.sh");
...
test("rewrites the mirror when the bundle changes and leaves it alone when it has not", () => {
  const {source, home} = bundle();
  const target = installOfficialSkills(source, home);
  const stamp = statSync(path.join(target, "computer-use", "SKILL.md")).mtimeMs;

  // A second launch against an unchanged bundle must not touch the files.
  installOfficialSkills(source, home);
  assert.equal(statSync(path.join(target, "computer-use", "SKILL.md")).mtimeMs, stamp);

  writeFileSync(path.join(source, "computer-use", "SKILL.md"), "---\nname: computer-use\n---\nnew\n");
  installOfficialSkills(source, home);
  assert.match(readFileSync(path.join(target, "computer-use", "SKILL.md"), "utf8"), /new/);
});

test("a mode change alone still republishes, so a script cannot lose its bit", () => {
  const {source, home} = bundle();
  const target = installOfficialSkills(source, home);
  chmodSync(path.join(target, "computer-use", "scripts", "run.sh"), 0o644);
  chmodSync(path.join(source, "computer-use", "scripts", "run.sh"), 0o700);
  installOfficialSkills(source, home);
  assert.equal(statSync(path.join(target, "computer-use", "scripts", "run.sh")).mode & 0o111, 0o100);
});
