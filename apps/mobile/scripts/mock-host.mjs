...
const port = Number(process.env.POLYMUX_MOBILE_MOCK_PORT || 47881);
const secret = process.env.POLYMUX_MOBILE_MOCK_SECRET || 'mobile-e2e-secret';
...
const messages = {
  'assistant-main': [
    ...
    },
    {
      ...
      content: 'Focus on **shipping the mobile companion**, reviewing Team handoffs, and keeping the Desktop Host online.',
...
const team = [{
  ...
  hostId: 'mobile-qa-host', hostName: 'MacBook Pro',
...
const hubChats = [
  ...
  },
  {
    ...
    preview: 'polymux-mobile-build.zip', lastActivity: new Date(Date.now() - 3_600_000).toISOString(), avatarUrl: fileTransferAvatar,
  },
];
const hubMessages = {
  'hub-family': [
  ...
  'hub-filehelper': [
    {
      ...
      body: 'polymux-mobile-build.zip', sentAt: new Date(Date.now() - 3_600_000).toISOString(), mine: true,
      attachments: [{id: 'attachment-1', name: 'polymux-mobile-build.zip', mimeType: 'application/zip', size: 204800}],
...
const server = createServer((request, response) => {
  ...
  if (request.method === 'GET' && request.url === '/polymux-host/v1/health') {
    reply(response, 200, {
      ok: true, hostId: 'mobile-qa-host', deviceName: 'Polymux QA Host', paired: true,
...
server.listen(port, '127.0.0.1', () => {
  process.stdout.write(`Polymux mobile mock Host listening on http://127.0.0.1:${port}\n`);
});
