<script lang="ts">
  ...
  import VaultScreen from './lib/VaultScreen.svelte';
  ...
  import {readCache, writeCache, type MobileCache} from './lib/cache';
  ...
  async function boot(): Promise<void> {
    try {
      ...
      if (connection?.accountUserId) {
        const {mobileAccount} = await import('./lib/account');
  ...
  async function disconnect(): Promise<void> {
    if (!confirm('Disconnect this mobile from the current Host?')) return;
    pollVersion += 1;
  ...
  function applyCache(cache: MobileCache): void {
...
{:else if mobileVault && !connection}
  <div class="mobile-vault">
    <header class="mobile-vault-bar">
      <button type="button" class="quiet" onclick={() => mobileVault = false}>Back</button>
      <strong>Vault</strong>
    </header>
    <div class="mobile-vault-body">
      <VaultScreen />
    </div>
  </div>
{:else if !connection && !showPairing}
  <SignInScreen onPair={() => showPairing = true} onVault={() => mobileVault = true} onConnect={async (host) => {
...
{:else if !connection}
  ...
  <PairScreen busy={pairing} error={pairError} onPair={connect} confirmationNumber={pairingNumber} onCancel={() => pairingAbort?.abort()} onVault={() => mobileVault = true} />
{:else}
  <div class:chat-open={connectedScreen === 'chat'} class="connected-shell">
    <ChatList
      ...
      vaultCall={(method, args) => connection ? rpc(connection, method, args ?? []) : Promise.reject(new Error('Host disconnected'))}
    />
...
<style>
  ...
  .mobile-vault-body { min-height: 0; flex: 1; display: flex; flex-direction: column; }
  .mobile-vault-bar strong { font-size: 1.0625rem; letter-spacing: -.02em; }
