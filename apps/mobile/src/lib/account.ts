...
const BROWSER_PREFIX = 'polymux-mobile-account:';
...
export function createMobileAccountStorage(): AccountStorage {
  return {
    async getItem(key) {
      if (isApplePhone()) {
...
let sharedAccount: ReturnType<typeof createMobileAccount> | undefined;
export function mobileAccount() {
  return sharedAccount ??= createMobileAccount();
}
...
export async function signInWithGoogle() {
  ...
  return account.client.signInWithOAuth('google', {
    redirectTo: 'polymux-mobile://auth/callback',
    openUrl: async url => (await invoke<{url: string}>('plugin:polymux-apple|oauth', {url})).url,
  });
}

export async function signOutMobileAccount() {
  ...
  const status = await mobileAccount()?.client.signOut();
  window.dispatchEvent(new Event('polymux-account-signed-out'));
  return status;
}

export function createMobileAccount(options: {
  url?: string | null;
  anonKey?: string | null;
  fetch?: AccountFetch;
  ...
  const client = new PolymuxAccountClient({
    ...
    storage: options.storage ?? createMobileAccountStorage(),
    fetch: options.fetch ?? transport(),
  });
  return {
    client,
    cloud: new FetchVaultCloud(client),
