...
import {createMobileAccountStorage, isApplePhone} from './account';
import {
...
const BROWSER_STORE_KEY = 'polymux-mobile-connection';
...
export async function pairAccountHost(host: import('./account-devices').AccountHost): Promise<SavedConnection> {
  ...
  const response = await transport()(`${host.public_endpoint}/polymux-host/v1/pair`, {
    ...
    body: JSON.stringify({accountSecret: host.pairing_secret, desktopId: mobileId, deviceName: mobileDeviceName(), deviceType: mobileDeviceType()}),
  ...
  const connection: SavedConnection = {
    ...
    mobileId, deviceName: host.device_name, pairedAt: value.pairedAt ?? new Date().toISOString(), accountUserId: host.user_id,
  };
...
export async function pairHost(endpointInput: string, codeInput: string, onPending?: (number: string) => void, signal?: AbortSignal): Promise<SavedConnection> {
  ...
  const mobileId = await getOrCreateDeviceId();
  const value = await requestPair(`${endpoint}/polymux-host/v1/pair`, code, mobileId, onPending, signal);
  return persistPairing(value, endpoint, mobileId);
}
...
export async function pairHostByCode(
  ...
  const code = pairingCode(codeInput);
  const mobileId = await getOrCreateDeviceId();
  const value = await requestPair(hostRelayCodePairEndpoint(relayOrigin), code, mobileId, onPending, signal);
  if (!value.hostId) throw new Error('The Host returned an incomplete identity.');
  return persistPairing(value, hostRelayPublicEndpoint(relayOrigin, value.hostId), mobileId);
}

async function requestPair(url: string, code: string, mobileId: string, onPending?: (number: string) => void, signal?: AbortSignal): Promise<PairResponse> {
  const response = await transport()(url, {
    method: 'POST',
    headers: {'content-type': 'application/json'},
    body: JSON.stringify({code, desktopId: mobileId, deviceName: mobileDeviceName(), deviceType: mobileDeviceType()}),
...
async function persistPairing(value: PairResponse, endpoint: string, mobileId: string): Promise<SavedConnection> {
  const connection: SavedConnection = {
    endpoint,
    secret: value.secret!,
    hostId: value.hostId!,
...
export async function rpc<T>(
  ...
  const response = await transport()(`${connection.endpoint}/polymux-host/v1/rpc`, {
    ...
    body: JSON.stringify({method, args, deviceType: mobileDeviceType()}),
...
export async function loadConnection(): Promise<SavedConnection | null> {
  if (isApplePhone()) {
    const secure = createMobileAccountStorage();
    const value = await secure.getItem('polymux-account:mobile-connection');
    ...
    if (connection) {
      await secure.setItem('polymux-account:mobile-connection', JSON.stringify(connection));
...
export async function saveConnection(connection: SavedConnection): Promise<void> {
  if (isApplePhone()) {
    await createMobileAccountStorage().setItem('polymux-account:mobile-connection', JSON.stringify(connection));
    const legacy = await store();
    await legacy.delete(CONNECTION_KEY);
    await legacy.save();
    return;
...
export async function clearConnection(): Promise<void> {
  if (isApplePhone()) {
    await createMobileAccountStorage().removeItem('polymux-account:mobile-connection');
    const legacy = await store();
    await legacy.delete(CONNECTION_KEY);
    await legacy.save();
    return;
...
function mobileDeviceName(): string {
  if (typeof navigator === 'undefined') return 'Polymux Mobile';
  ...
  return 'Polymux Mobile';
}

export function mobileDeviceType(userAgent = typeof navigator === 'undefined' ? '' : navigator.userAgent, touchPoints = typeof navigator === 'undefined' ? 0 : navigator.maxTouchPoints): 'mobile' | 'tablet' {
  return /iPad|Tablet/i.test(userAgent) || /Macintosh/.test(userAgent) && touchPoints > 1 || /Android/.test(userAgent) && !/Mobile/.test(userAgent) ? 'tablet' : 'mobile';
}
