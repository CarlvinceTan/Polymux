...
import {normalizeEndpoint, pairHostByCode, mobileDeviceType} from './host.js';
...
test('connect code pairs through the Polymux domain and saves the stable Host endpoint', async () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(values.get('polymux-mobile-connection') ?? '{}').hostId, paired.hostId);
  } finally {
...
test('an old immediate-grant response and a cancelled request never save a connection', async () => {
  ...
  try {
    ...
    assert.equal(saved.includes('polymux-mobile-connection'), false);
  } finally {
    globalThis.fetch = originalFetch;
...
test('mobile metadata distinguishes mobiles, Android tablets and iPad desktop-mode browsers', () => {
  assert.equal(mobileDeviceType('iPhone', 5), 'mobile');
  assert.equal(mobileDeviceType('Android Mobile', 5), 'mobile');
  assert.equal(mobileDeviceType('Android', 5), 'tablet');
  assert.equal(mobileDeviceType('iPad', 5), 'tablet');
  assert.equal(mobileDeviceType('Macintosh', 5), 'tablet');
});
