<script lang="ts">
  ...
  import {mobileAccount, isApplePhone, signInWithApple, signInWithGoogle, signOutMobileAccount} from './account';
  import {discoverAccountHosts, type AccountHost} from './account-devices';

  export let onPair: () => void;
  export let onVault: () => void;
...
<main class="signin">
  <section aria-labelledby="signin-title">
    ...
    {#if signedIn}
      ...
      <button class="quiet" disabled={busy} onclick={() => void run(async () => { await signOutMobileAccount(); signedIn = false; hosts = []; })}>Sign out</button>
    {:else if account}
    ...
    <div class="links"><button class="quiet" disabled={busy} onclick={onPair}>Connect with QR or code</button><button class="quiet" disabled={busy} onclick={onVault}>Open Vault</button></div>
  </section>
</main>
