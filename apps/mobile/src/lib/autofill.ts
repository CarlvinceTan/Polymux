...
import type {JsonValue, VaultListDto, VaultSecretsDto} from '@polymux/protocol';
...
export async function updateAutoFill(call: (method: string, args?: JsonValue[]) => Promise<unknown>) {
  ...
  const list = await call('vault.list') as VaultListDto;
  const entries = [];
  for (const item of list.items) {
    if (!item.hasPassword && !item.hasTotp) continue;
    const secrets = await call('vault.reveal', [item.id]) as VaultSecretsDto;
    const otpAuth = item.hasTotp ? await call('vault.otpauth', [item.id]) as string : null;
