...
>
  <button type="button" aria-expanded={open} aria-controls="product-menu-panel" onclick={() => open = !open}>
    <span>Product</span>
    <svg class="product-chevron" viewBox="0 0 20 20" fill="none" aria-hidden="true"><path d="m5 7 5 5 5-5" /></svg>
  </button>
...
<style>
  .product-menu{position:relative;display:flex;align-items:center;color:#969691;font:inherit}.product-menu>button{min-height:36px;padding:8px 12px;display:inline-flex;align-items:center;gap:6px;border:0;background:transparent;color:inherit;font:inherit;cursor:pointer;transition:color 140ms ease}.product-menu>button:hover,.product-menu.open>button,.product-menu.active>button{color:var(--ink,#171717)}.product-menu.active>button:after{content:"";position:absolute;right:12px;bottom:4px;left:12px;height:2px;background:var(--ink,#171717)}.product-chevron{width:15px;height:15px;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round;transition:transform 140ms ease}.open .product-chevron{transform:rotate(180deg)}
