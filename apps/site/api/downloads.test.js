...
import handler from './downloads.js';
...
test('maps the latest release to native installers for each platform', async (context) => {
  context.mock.method(globalThis, 'fetch', async () => ({
    ...
    json: async () => ({
      ...
      assets: [
        ...
        {name: 'polymux-desktop_0.3.0_amd64.deb', browser_download_url: 'https://files/deb', size: 3},
        {name: 'polymux-desktop-0.3.0.x86_64.rpm', browser_download_url: 'https://files/rpm', size: 4},
      ],
    }),
  }));
  ...
  assert.equal(result.body.platforms.linux.url, 'https://files/deb');
  assert.equal(result.body.platforms.linuxRpm.url, 'https://files/rpm');
});
