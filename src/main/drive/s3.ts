...
export class S3Drive implements DriveAdapter {
  ...
  async upload(parentPath: string, localPath: string): Promise<DriveEntryDto> {
    ...
    const key = `${this.#key(parentPath)}${name}`;
    await this.#send("PUT", key, undefined, bytes);
    return {
      id: key,
      name,
      kind: "file",
  ...
  async download(target: string, destination: string): Promise<void> {
    const response = await this.#send("GET", this.#key(target, false));
  ...
  async #send(
    ...
    if (!response.ok) throw await requestError(response, "The S3 request");
    return response;
  }
}
