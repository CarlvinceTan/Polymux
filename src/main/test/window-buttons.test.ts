...
class ResettingMacWindow implements MacWindowButtons {
  visible = true;
  position: {x: number; y: number} = {...MIDAS_TRAFFIC_LIGHT_POSITION};
  calls: string[] = [];
...
test("keeps native traffic lights at the Midas position across focus cycles", () => {
  ...
  assert.deepEqual(window.position, MIDAS_TRAFFIC_LIGHT_POSITION);
