...
def _detach_capture(process):
    if _detached.is_set():
        return True
    state = process.GetState()
    if state in (lldb.eStateDetached, lldb.eStateExited):
        _detached.set()
        return True
...
def _watch_capture(process, parent_pid, duration):
    deadline = time.monotonic() + duration
    while not _detached.wait(0.1):
        if os.getppid() != parent_pid or time.monotonic() >= deadline:
            if _detach_capture(process):
                return


def _auto_detach(debugger, command, result, _dict):
    global _capture_owner, _capture_deadline
    duration = max(1, min(float(command.strip() or "60") - 5, 55))
    _capture_owner = os.getppid()
    _capture_deadline = time.monotonic() + duration
    _detached.clear()
    # wechatd ends its command file with `continue`. A synchronous continue
    # prevents a watchdog thread from acquiring LLDB's API lock. Own the run
    # and detach in one scripted command, with asynchronous process execution.
    debugger.HandleCommand("command unalias continue")
    debugger.HandleCommand("command alias continue cdn-capture-run")
    result.AppendMessage("cdn capture auto-detach armed")


def _run_capture(debugger, command, result, _dict):
    if _capture_owner is None or _capture_deadline is None:
        result.SetError("cdn capture cleanup is not armed")
        return
    process = debugger.GetSelectedTarget().GetProcess()
    debugger.SetAsync(True)
    if os.getppid() == _capture_owner and time.monotonic() < _capture_deadline:
        continued = process.Continue()
        if not continued.Success():
            result.SetError("cdn capture could not continue")
    _watch_capture(process, _capture_owner,
                   max(0, _capture_deadline - time.monotonic()))
    # A nested LLDB `quit` does not end the outer command interpreter. Exit
    # only after successful detachment (or target exit), never while attached.
    # This helper runs exclusively in wechatd's disposable LLDB subprocess.
    os._exit(0)


def __lldb_init_module(debugger, _dict):
    debugger.HandleCommand(
        f"command script add -f {__name__}._run_capture cdn-capture-run"
    )
    debugger.HandleCommand(
