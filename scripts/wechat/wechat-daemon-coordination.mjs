...
/** The embedded Hub owns relay recovery around a native writer call. In that
 * mode the driver only has to start wechatd again; waiting for its debugger to
 * become observable would hold a verified send open while the Hub is already
 * responsible for bringing the relay back. Standalone callers retain the
 * stricter ready-before-return behaviour. */
export function waitsForDaemonRecovery(environment = process.env) {
  return environment.POLYMUX_WECHAT_BRIDGE_MANAGED_RECOVERY !== "1";
}
