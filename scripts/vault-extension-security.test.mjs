...
function nativeRequest(home, caller, {instance, id = extensionId, browser = "chromium", manifestPath = join(home, "Library", "Application Support", "Mozilla", "NativeMessagingHosts", "com.polymux.tab_context.json")} = {}) {
  const body = Buffer.from(JSON.stringify({type: "polymux:vault-connection"}));
...
test("native messaging returns the private capability only to the installed extension and honors isolate paths", () => {
  const home = mkdtempSync(join(tmpdir(), "polymux-native-vault-"));
  try {
    mkdirSync(join(home, ".polymux"));
    const file = join(home, ".polymux", "vault-extension-capability");
    ...
    writeFileSync(join(home, ".polymux-review", "vault-extension-capability"), "d".repeat(64), {mode: 0o600});
...
test("Vault API obtains capabilities only from native messaging, protects destinations and retries a rotated token", async () => {
  ...
  assert.deepEqual(await api.vaultRequest("/v1/vault/status", {headers: {Authorization: "attacker"}}), {unlocked: true});
  assert.equal(nativeCalls, 2);
  assert.equal(calls[0].init.headers.Authorization, `Bearer ${token}`);
  assert.equal(calls[1].init.headers.Authorization, `Bearer ${"c".repeat(64)}`);
  assert.equal(calls.every(({url}) => url === "http://127.0.0.1:47654/v1/vault/status"), true);
  await assert.rejects(api.vaultRequest("https://attacker.example/v1/vault/status"), /Invalid Vault endpoint/);
  await assert.rejects(api.vaultRequest("/v1/snapshot"), /Invalid Vault endpoint/);
...
test("missing native enrollment fails closed without an HTTP request", async () => {
  ...
  await assert.rejects(api.vaultRequest("/v1/vault/status"), (error) => error.code === "DESKTOP_OFFLINE");
...
function backgroundFixture() {
  ...
  vm.runInNewContext(text, {chrome, URL, Headers, extensionProtocolHeaders: () => ({}),
    ...
    vaultRequest: async () => { calls.push("vault-request"); return {}; },
...
test("background uses the real frame sender, rejects opaque/missing origins and reserves unlock/proxy to extension pages", async () => {
  ...
  for (const type of ["polymux:vault-request", "polymux:vault-device-unlock", "polymux:vault-pending"])
    assert.equal((await fixture.request({type, path: "/v1/vault/export", password: "secret"}, sender)).ok, false);
  assert.match((await fixture.request({type: "polymux:webauthn", action: "unlock"}, sender)).error, /Invalid passkey/);
  assert.equal(fixture.calls.length, 1);
  assert.equal((await fixture.request({type: "polymux:vault-device-unlock", password: "secret"}, {id: extensionId, url: `chrome-extension://${extensionId}/vault/popup.html`})).unlocked, true);
...
function bridgeFixture(kind, answer) {
  ...
  let text = source(kind === "desktop" ? "apps/desktop/src/preload/webauthn.ts" : "apps/extension/vault/webauthn.js");
...
for (const kind of ["desktop", "extension"]) {
  test(`${kind} passkey bridge whitelists hostile page options before privileged dispatch`, async () => {
    ...
    await fixture.send("get", {action: "create", type: "polymux:vault-request", origin: "https://attacker.example", path: "/v1/vault/export", password: "never-forward", itemId: "forged", rpId: "real.example", challenge: "abc", allowCredentialIds: ["id", 5]});
  ...
  test(`${kind} locked passkeys display only trusted-surface unlock instructions and Retry`, async () => {
    ...
    assert.ok(shadow);
    assert.match(shadow.innerHTML, /Unlock Vault in Polymux/);
...
test("Firefox enrollment pins both its add-on ID and native manifest and uses a separate wrapper", () => {
  const home = mkdtempSync(join(tmpdir(), "polymux-firefox-vault-"));
  try {
    mkdirSync(join(home, ".polymux"));
    writeFileSync(join(home, ".polymux", "vault-extension-capability"), token, {mode: 0o600});
...
test("Safari explicitly reports standalone availability and never requests a desktop capability", async () => {
  ...
  assert.match(api.desktopVaultUnavailableReason(), /Desktop Vault connection is not available/);
  await assert.rejects(api.vaultRequest("/v1/vault/status"), (error) => error.code === "DESKTOP_OFFLINE");
