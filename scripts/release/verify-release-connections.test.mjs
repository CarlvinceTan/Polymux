...
for (const missing of ["POLYMUX_SUPABASE_URL", "POLYMUX_SUPABASE_ANON_KEY"]) {
  test(`account sign-in config is required: ${missing}`, () => {
    const env = cleanEnvironment();
    for (const name of required) env[name] = "configured";
    delete env[missing];
    const result = spawnSync(process.execPath, [script.pathname], {encoding: "utf8", env});
    assert.equal(result.status, 1);
