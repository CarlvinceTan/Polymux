...
test("production Locker builds write account config only into staging", () => {
  const stage = mkdtempSync(path.join(tmpdir(), "polymux-config-test-"));
  try {
    const result = spawnSync(process.execPath, [path.join(root, "apps/extension/scripts/build-locker.mjs"), "--output-dir", stage], {cwd: tmpdir(), env, encoding: "utf8"});
    assert.equal(result.status, 0, result.stderr);
    const config = readFileSync(path.join(stage, "locker/config.js"), "utf8");
    assert.match(config, /https:\/\/example\.supabase\.co/);
    assert.match(config, /packaging-public-test-key/);
    assert.ok(readFileSync(path.join(stage, "locker/offline.js")).length > 100_000);
    const invalid = spawnSync(process.execPath, [path.join(root, "apps/extension/scripts/build-locker.mjs"), "--output-dir", stage], {env: {...env, POLYMUX_SUPABASE_URL: "http://insecure.example"}, encoding: "utf8"});
