 ...
 * compression quality so Mobile can carry small frames over a wireless route. */
...
export async function buildMobileWda(options = {}) {
  if (process.platform !== "darwin")
    throw new Error("The unsigned iPhone WDA runtime must be built on macOS with Xcode.");
  const outputDirectory = options.outputDirectory ?? path.join(root, "resources", "mobile", "ios");
  ...
  const staging = await mkdtemp(path.join(tmpdir(), "polymux-mobile-wda-"));
