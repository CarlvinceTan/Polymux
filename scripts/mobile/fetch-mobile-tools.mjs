#!/usr/bin/env node
/**
 * Prepares the pinned Android mobile runtime bundled with Polymux.
 *
 ...
 * archives here keeps Mobile usable without Android Studio, a package manager,
...
export const MOBILE_TOOL_ARCHIVES = {
  "darwin-arm64": {
...
export function mobileToolsTarget(platform, arch) {
  const key = `${platform}-${arch}`;
  const target = MOBILE_TOOL_ARCHIVES[key];
  if (!target)
    throw new Error(`No pinned Android mobile runtime for ${key}`);
  return {
    ...target,
    url:
...
export function mobileRuntimeFileNames(names, platform) {
...
export async function fetchMobileTools(options = {}) {
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  const target = mobileToolsTarget(platform, arch);
  const outputDirectory = options.outputDirectory ??
    path.join(root, "resources", "mobile", "android");
  if (!options.force && await ready(outputDirectory, platform, arch)) {
    console.log(`Android mobile tools ${runtimeStamp(platform, arch)} already present, skipping`);
  ...
  const staging = await mkdtemp(path.join(tmpdir(), "polymux-mobile-android-"));
  ...
  try {
    ...
    const files = mobileRuntimeFileNames(names, platform);
  ...
  console.log(
    `Android mobile tools ${runtimeStamp(platform, arch)} → ${path.relative(root, outputDirectory)}`,
  );
  return outputDirectory;
}
