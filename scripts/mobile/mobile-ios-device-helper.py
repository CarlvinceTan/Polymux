...
class MobileDeviceError(Exception):
    """A concise error safe to show in the Mobile UI."""
...
async def polymux_no_root_provider(
    ...
    global _TUNNEL_UNDERLAY, _TUNNEL_UNDERLAY_SCOPE
    """Prefer USB CoreDeviceProxy, but also support a genuinely unplugged mobile.
    ...
    an iOS 17.4+ mobile that is already unplugged never reaches its valid Wi-Fi
...
async def _open_tunnel(udid: str, transport: str) -> tuple[Any, Any, str]:
    ...
    if transport == "native":
        raise MobileDeviceError("Apple's native iPhone tunnel is only available on macOS.")
...
async def install_app(
    ...
    if not app.is_dir() or app.suffix != ".app":
        raise MobileDeviceError("The signed iPhone control app is missing.")
    configure_pairing_home(pairing_directory)
    tunnel, rsd, route = await _open_tunnel(udid, transport)
    try:
        from pymobiledevice3.services.installation_proxy import InstallationProxyService
...
async def run_wda(
    ...
    try:
        ...
        while asyncio.get_running_loop().time() < deadline:
            ...
            raise MobileDeviceError("Timed out while starting iPhone control.")
        ...
        async def watch_health() -> None:
            ...
            while True:
                ...
                failures += 1
                if failures >= 2:
                    raise MobileDeviceError("The iPhone wireless tunnel became unavailable.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="polymux-mobile-ios-device")
...
async def _main(args: argparse.Namespace) -> None:
    ...
    if not 0 <= args.local_port <= 65535:
        raise MobileDeviceError("The local iPhone relay port is invalid.")
    await run_wda(
        args.udid,
        args.bundle_id,
        args.local_port,
...
if __name__ == "__main__":
    try:
        ...
        if os.environ.get("POLYMUX_MOBILE_DEBUG") == "1":
            raise
