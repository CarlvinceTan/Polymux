...
export const IOS_DEVICE_TARGETS = {
  "darwin-arm64": {
    lock: "mobile-ios-device-darwin-arm64.lock",
  ...
  "win32-x64": {
    lock: "mobile-ios-device-win32-x64.lock",
  ...
  "linux-x64": {
    lock: "mobile-ios-device-linux-x64.lock",
...
async function ready(outputDirectory, platform, arch, target, helperBytes, lockBytes) {
  try {
    ...
    await Promise.all([
      ...
      stat(path.join(outputDirectory, "LICENSE.mobile-ios-device-helper.txt")),
...
export async function buildMobileIosDevice(options = {}) {
  ...
  const outputDirectory = options.outputDirectory ?? path.join(root, "resources", "mobile", "ios", "device");
  const helperBytes = await readFile(path.join(root, "scripts", "mobile", "mobile-ios-device-helper.py"));
  const lockBytes = await readFile(path.join(root, "scripts", "mobile", target.lock));
  ...
  const staging = await mkdtemp(path.join(tmpdir(), "polymux-mobile-ios-device-"));
  ...
  try {
    ...
    await copyFile(path.join(root, "scripts", "mobile", "mobile-ios-device-helper.py"), path.join(pending, "helper.py"));
    ...
    await writeFile(path.join(pending, "LICENSE.mobile-ios-device-helper.txt"), license);
