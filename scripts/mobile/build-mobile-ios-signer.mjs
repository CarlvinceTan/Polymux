...
async function ready(outputDirectory, platform, arch, target) {
  try {
    const [stamp, manifest, helper] = await Promise.all([
      ...
      readFile(path.join(root, "scripts", "mobile", "mobile-ios-signer-helper.py")),
    ]);
    if (
...
export async function buildMobileIosSigner(options = {}) {
  ...
  const outputDirectory = options.outputDirectory ??
    path.join(root, "resources", "mobile", "ios", "signer");
  ...
  const helperBytes = await readFile(path.join(root, "scripts", "mobile", "mobile-ios-signer-helper.py"));
  const downloads = [
  ...
  const staging = await mkdtemp(path.join(tmpdir(), "polymux-mobile-ios-signer-"));
  ...
  try {
    ...
    if (target.zsignArchive) {
      ...
      const sourceDirectory = options.zsignDirectory ??
        path.join(root, "resources", "mobile", "ios", "signer-tools");
