...
class MobileSignerError(Exception):
...
def _signer_data_dir() -> Path:
    ...
    return Path(base) / "PolymuxMobileSigner"
...
def _ensure_unicorn() -> None:
    ...
    if wheel is None:
        raise MobileSignerError("Local Apple sign-in is not available on this computer yet.")
    target = _signer_data_dir() / "vendor" / f"unicorn-{UNICORN_RELEASE}"
    marker = target / ".complete"
    if not marker.exists():
        target.parent.mkdir(parents=True, exist_ok=True)
        request = urllib.request.Request(wheel[0], headers={"User-Agent": "Polymux/mobile-signer"})
        with urllib.request.urlopen(request, timeout=60) as response:
            data = response.read()
        if hashlib.sha256(data).hexdigest() != wheel[1]:
            raise MobileSignerError(
...
def _native_macos_anisette_headers() -> dict[str, str]:
    ...
    try:
        ...
        raise MobileSignerError("macOS returned invalid Apple sign-in data.") from exc
    ...
    if not all(isinstance(values.get(key), str) and values[key] for key in required):
        raise MobileSignerError("macOS did not return complete Apple sign-in data.")
...
def _load_engine() -> None:
    ...
    paths.APP_NAME = "PolymuxMobileSigner"
    provision.CERTIFICATE_MACHINE_NAME = "Polymux Mobile"
    if sys.platform == "darwin":
...
def _verified_anisette_libraries() -> io.BytesIO:
    ...
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "Polymux/mobile-signer"},
    )
...
def _sign_wda(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if not source.is_dir() or source.suffix != ".app":
        raise MobileSignerError("The bundled WebDriverAgent app is missing.")
    if not zsign.is_file():
        raise MobileSignerError("The bundled iPhone signer is missing.")
    if not udid:
        raise MobileSignerError("Connect and unlock the iPhone first.")
    if output == source or output.suffix != ".app":
        raise MobileSignerError("The WebDriverAgent output path was invalid.")

    bundle_id = provision.team_scoped_bundle_id(BASE_WDA_BUNDLE_ID)
    bundle = provision.ensure_signing_assets(
        bundle_id,
        udid,
        app_name="Polymux Mobile Control",
    ...
    if completed.returncode != 0:
        ...
        raise MobileSignerError(f"WebDriverAgent signing failed: {suffix}")
    return {
        "bundleId": bundle_id,
        "output": str(output),
...
def _handle(method: str, params: dict[str, Any]) -> Any:
    ...
    if method == "beginLogin":
        ...
        if not email or not password:
            raise MobileSignerError("Enter your Apple Account email and password.")
    ...
    if method == "complete2fa":
        ...
        if _pending_password is None:
            raise MobileSignerError("Start Apple Account sign-in again.")
        if not code:
            raise MobileSignerError("Enter the verification code Apple sent to your trusted device.")
    ...
    raise MobileSignerError(f"Unknown mobile signer operation: {method}")
...
def _response(request: Any) -> dict[str, Any]:
    ...
    try:
        if not isinstance(request, dict):
            raise MobileSignerError("The mobile signer request was invalid.")
