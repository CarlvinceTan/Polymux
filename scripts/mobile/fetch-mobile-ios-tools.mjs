...
export function goIosTarget(platform, arch) {
  ...
  if (!target) throw new Error(`No pinned iOS mobile runtime for ${key}`);
  return target;
}
...
export async function fetchMobileIosTools(options = {}) {
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  const target = goIosTarget(platform, arch);
  const outputDirectory = options.outputDirectory ?? path.join(root, "resources", "mobile", "ios", "tools");
  if (!options.force && await ready(outputDirectory, platform, arch, target)) {
    console.log(`iOS mobile tools ${runtimeStamp(platform, arch)} already present, skipping`);
  ...
  const staging = await mkdtemp(path.join(tmpdir(), "polymux-mobile-ios-"));
  ...
  console.log(`iOS mobile tools ${runtimeStamp(platform, arch)} → ${path.relative(root, outputDirectory)}`);
  return outputDirectory;
}
