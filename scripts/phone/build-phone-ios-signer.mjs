...
async function ready(outputDirectory, platform, arch, target) {
  try {
    const [stamp, manifest, helper] = await Promise.all([
      ...
      readFile(path.join(root, "scripts", "phone", "phone-ios-signer-helper.py")),
    ]);
    if (
...
export async function buildPhoneIosSigner(options = {}) {
  ...
  const helperBytes = await readFile(path.join(root, "scripts", "phone", "phone-ios-signer-helper.py"));
  const downloads = [
