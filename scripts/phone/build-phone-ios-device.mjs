...
export async function buildPhoneIosDevice(options = {}) {
  ...
  const helperBytes = await readFile(path.join(root, "scripts", "phone", "phone-ios-device-helper.py"));
  const lockBytes = await readFile(path.join(root, "scripts", "phone", target.lock));
  ...
  try {
    ...
    await copyFile(path.join(root, "scripts", "phone", "phone-ios-device-helper.py"), path.join(pending, "helper.py"));
